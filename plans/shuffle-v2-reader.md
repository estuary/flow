...
4. Build visibility index via sorted merge-join:
   ...
   - Merge-join with Frontier:
     - The Frontier is sorted by (binding, journal). Block journals are
       sorted by journal_name only (no binding). A journal name may appear
       under multiple bindings in the Frontier.
     - Walk Frontier entries linearly. For each entry, find the matching
       ...
       delta-decoding on demand). When the Frontier's binding increases,
       re-seek into the block journal list from the beginning, since
       the same journal name may appear under a different binding.
       This makes the journal scan O(F_j × B_j) in the worst case where
       every journal appears under every binding, but in practice the
       number of bindings per journal is nearly always 1.
     - For each (binding, block_journal) match, merge-join the Frontier's
