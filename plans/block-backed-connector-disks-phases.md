...
- `gazette::uuid` (`crates/proto-gazette/src/uuid.rs`) provides `Producer` /
  ...
  replay reader builds on it. Spill/buffering of pending transactions is
...
2. **The proto follows the standard pipeline.** `go/protocols/disk/disk.proto`
   ...
   a `disk_client` / `disk_server` feature pair; Go bindings generated by
   adding the file to `mise/tasks/build/go-protobufs` for pipeline symmetry
   (nothing Go-side consumes them yet). `mise run build:rust-protobufs`
...
4. **Privileged tests run in the normal suite.** No special profile or
   ...
   `sudo -n` — a scenario-helper binary while the device server is being
   built (Phases 2–3), the `flow-disk-daemon` binary itself from Phase 4 on,
   so tests exercise the daemon exactly as it ships. `cargo nextest run -p
...
5. **Tunables are configuration, not constants.** As a general-purpose tool
   ...
   - *Policy, safe to change between daemon restarts* — compaction (`k`,
     `r`, horizon minimum) and record/batch append bounds are daemon flags
     with the design's values as defaults. Replay derives all state from
...
6. **Credentials are session-supplied or absent; the daemon mints none.** A
   ...
   The endpoint is the one part with a config fallback, because a stand-alone
   deployment can then point the daemon at its cluster once and have clients
   name only journals. No endpoint from either source fails the session.
7. **The daemon owns the recovery floor label, end to end.** It derives the
   ...
   The label *name* is a daemon flag with **no default** — a general-purpose
   daemon should carry no system's label vocabulary, and
   `estuary.dev/truncated-at` is Flow's. The daemon refuses to start unless
   given either a name or an explicit flag disabling floor labels, because
   silent absence is the dangerous state: everything still works, but every
   recovery replays from the earliest fragment and the required journal range
   grows without bound. Flow's deployment sets the name; a stand-alone
   operator picks their own or opts out.
...
8. **The daemon creates the journal, lazily, from typed inputs.** Decision 7
   ...
   Everything exposed defaults from daemon configuration when unset, so the
   platform can pin a value (Flow's five-minute refresh interval, say) without
   the protocol hardcoding it.

   Unset fields fall back to daemon configuration, so a stand-alone deployment
   passes a journal name and little else. That fallback is active only when a
   store is configured: no configured store and none supplied is a terminal
   error, never a journal whose fragments land somewhere unintended. Flow
   configures no store, so an omitted `JournalConfig` fails loudly at `Open`
   instead of misplacing a tenant's data.
...
**Session with config fallback.** The session wins; configuration fills in an
unset field. Every field of `JournalConfig` except the journal name, plus
`Broker.endpoint`. Zero or empty is the unset marker throughout, which is
unambiguous because zero is invalid for each: Gazette rejects a zero
replication, fragment length, or refresh interval, and `CompressionCodec` zero
is `INVALID`. Two fallbacks must be explicit rather than silent — neither a
fragment store nor an endpoint resolving from either source is a terminal
error.
...
**Config-only, host facts.** Socket path, image directories, spill directory,
admin bind address, log format, owner-pool size, per-device queue depth,
`mkfs`/mount timeouts, SIGTERM drain timeout. Image directories are a *list*
the daemon chooses from, so a host with several NVMe drives spreads disks
across them without a client ever knowing host topology.

**Config-only, policy.** Compaction `k`, `r`, and horizon minimum; append
record and batch bounds; the floor label name or its explicit opt-out.
...
mutation when nothing publishes); record/batch bounds respected across a
large synthetic delta; commit → read back → sequenced decode yields exactly
...
**Build:**
- `flow-disk-daemon` binary: clap args (Unix-socket path, image/spill
  directories, admin port, log format, decision-5 policy tunables, and
  decision-6 broker defaults, decision-7 floor-label name or its explicit
  opt-out), tonic server on the socket
...
a session against an
unauthenticated broker (endpoint from daemon flags, no `Broker` in `Open` and
none sent later, so an anonymous client) works end to end; violation matrix returns
...
- Replay reader over a fixed range: after fencing and repair, obtain
  ...
  `gazette::uuid::sequence` per producer; spill the at-most-one
  unacknowledged delta (bounded spill file); apply acknowledged deltas in
...
content-identical filesystems; ext4 journal replay on a mid-flush cut
...
- Hardening: graceful drain on SIGTERM (sessions end, devices torn down);
  bounded per-session memory (spill to disk where the design allows);
...
**Tests:** soak — many concurrent disks per owner pool under mixed
read/write/discard load with periodic publish/commit and randomized
kill/recover, asserting content equality and no fd/device/mount leaks over
the run; fault injection — broker outage mid-delta (backpressure stalls
