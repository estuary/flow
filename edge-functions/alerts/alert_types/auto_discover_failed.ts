...
type AutoDiscoverFailedRecord = AlertRecord<
  "auto_discover_failed",
  ControllerAlertArguments
>;

const formatAlertEmail = ({
  arguments: { recipients, spec_type },
  catalog_name,
}: AutoDiscoverFailedRecord): EmailConfig[] => {
  const subject = `Estuary Flow: Auto-discover failed for ${spec_type} ${catalog_name}`;
  ...
  return recipients.map((recipient) => ({
    content: commonTemplate(
      `
        <mj-text>
          Your Estuary ${spec_type} <a class="identifier">${catalog_name}</a> has encountered errors during the auto-discover process. This means Estuary was unable to automatically detect and update changes to your source schema or tables. To troubleshoot please:
        </mj-text>
        <mj-text>
          <ul>
...
const formatConfirmationEmail = ({
  ...
  const subject = `Estuary Flow: Auto-discover resolved for ${spec_type} ${catalog_name}`;
  const detailsPageURL = getTaskDetailsPageURL(catalog_name, spec_type);
  ...
  return recipients.map((recipient) => ({
    content: commonTemplate(
      `
        <mj-text>
          Good news! The auto-discover alert for your ${spec_type} <a class="identifier">${catalog_name}</a> has been resolved.
        </mj-text>
        <mj-text>
...
export const autoDiscoverFailedEmail = (
  request: AutoDiscoverFailedRecord,
): EmailConfig[] => {
