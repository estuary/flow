...
function validateAlertRecordKeys<T extends keyof typeof emailTemplates>(
  request: any,
): request is Parameters<(typeof emailTemplates)[T]>[0] {
  ...
  const validFiredAtTimestamp =
    Object.hasOwn(request, "fired_at") && typeof request.fired_at === "string";

  const validResolvedAtTimestamp =
    Object.hasOwn(request, "resolved_at") &&
    (typeof request.resolved_at === "string" || request.resolved_at === null);
...
const emailNotifications = (
  ...
  const notificationPromises = pendingNotifications.flatMap(
...
Deno.serve({ port: 8000 }, async (rawRequest: Request): Promise<Response> => {
    ...
    return new Response(
      JSON.stringify({
        error: {
          ...
          description: `You must provide 'alert_type', 'catalog_name', 'fired_at', 'resolved_at', and 'arguments'. 'alert_type' must be one of [${Object.keys(
            emailTemplates,
          ).join(", ")}]`,
        },
      }),
