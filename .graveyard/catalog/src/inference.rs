use crate::{Schema, Scope, DB};
use doc::{inference::Shape, Pointer};
...
pub fn register_all(scope: Scope, schema_uri: &Url) -> crate::Result<()> {
...
pub fn register_one(db: &DB, schema_uri: &str, inference: &Inference) -> crate::Result<()> {
...
mod test {
    ...
    use json::schema::{build::build_schema, index::Index};
    ...
    fn inferences_are_registered_by_schema_uri() {
        ...
        let actual = crate::dump_table(&db, "inferences").unwrap();
