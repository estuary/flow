...
impl Shuffle {
    pub async fn run(&self, ctx: &mut crate::CliContext) -> anyhow::Result<()> {
        ...
        let service_even = shuffle::Service::new(peer_addr_even.clone());
        let service_odd = shuffle::Service::new(peer_addr_odd.clone());
...
fn build_member_topology(count: u32, addr_even: &str, addr_odd: &str) -> Vec<Member> {
    ...
    for i in 0..count {
        ...
        let address = if i % 2 == 0 { addr_even } else { addr_odd };
