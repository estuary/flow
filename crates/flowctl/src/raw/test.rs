//! `flowctl raw test` — run a catalog's tests locally on the runtime-next stack via
//! the `catalog-tests` crate. No Gazette broker and no etcd are involved:
//! derivations run as resident runtime-next sessions (derive-sqlite in-process,
//! image derivations as containers), and tests execute against an in-memory
//! collection store.
...
impl Test {
    pub async fn run(&self, _ctx: &mut crate::CliContext) -> anyhow::Result<()> {
        ...
        // Build to `Validations` (including built tests) fully locally, with no
        ...
        let results = catalog_tests::run_tests(&built.built, options).await?;
        render(&results);
...
/// Print the run's summary: one line per test, then a tally.
fn render(results: &catalog_tests::TestResults) {
    let color = std::io::stdout().is_terminal();

    for outcome in &results.outcomes {
        match &outcome.status {
            catalog_tests::TestStatus::Passed => {
                println!("✔️  {}", green(&outcome.name, color))
            }
            catalog_tests::TestStatus::Failed { error } => {
                let (path, ptr) = scope_to_path_and_ptr(&outcome.scope);
                println!(
                    "❌ {} — failed at {}#{}:",
                    red(&outcome.name, color),
                    path,
                    yellow(&ptr, color)
                );
            ...
            catalog_tests::TestStatus::NotRun { reason } => {
                println!("⏭️  {} — not run ({reason})", yellow(&outcome.name, color))
            }
        }
    }

    let not_run = match results.not_run() {
        0 => String::new(),
        n => format!(", {n} not run"),
    };
    println!(
        "\nRan {} tests: {} passed, {} failed{not_run}",
        results.outcomes.len(),
        results.passed(),
        results.failed(),
    );
