...
pub struct Preview {
    ...
    /// encouraging reductions. With `--fixture`, flowctl sleeps between fixture
    /// transactions. Otherwise the delay raises the task's minimum transaction
...
/// and the preview-rendering observer factory installed on the shard (and, for
/// materializations / derivations, the leader) Service. The observer carries the
...
struct Controls {
    ...
    observer_factory: observe::PreviewObserverFactory,
}
...
impl Preview {
    pub async fn run(&self, ctx: &mut crate::CliContext) -> anyhow::Result<()> {
        ...
        let controls = Controls {
            initial_state_json,
            publisher_factory: publish::PreviewPublisherFactory,
            observer_factory: observe::PreviewObserverFactory::new(
                log_handler,
                *output_state,
                *output_apply,
            ),
...
fn prepare_sessions<S>(
    ...
    let (targets, dirs, plan) = start_fixtures(run, task, path, session_targets, delay)?;
    Ok((
        targets,
        dirs,
        stop_token.clone(),
...
async fn emit_final_connector_state(run: &services::Run) -> anyhow::Result<()> {
    let state = runtime_next::read_final_connector_state(proto_flow::runtime::RocksDbDescriptor {
    ...
    observe::emit_final_state(&state);
    Ok(())
}
