...
pub struct Preview {
    ...
    /// of transactions for each session? A session is stopped and the next started
    /// upon reaching the target number of transactions, or upon a timeout, or if
    /// the connector exits (when a capture).
    ///
    /// The default is a single session with an unbounded number of transactions.
    /// For a given session, a value less than zero means "unlimited transactions",
    /// though the session will still end upon a connector EOF or timeout.
