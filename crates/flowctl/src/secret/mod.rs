...
/// Shadows `crate::graphql::JSON` for this module's queries: a wrapped
/// document is handled as a `Value` so that its rendering is sorted, matching
/// what config-encryption seals and what `setSecret` stores.
...
pub enum Command {
    ...
    ///
    /// The value is read from stdin, or from `--from-file`. It's interpreted
    /// as a string with whitespace trimmed, unless `--json` is given.
    ///
...
async fn do_set(args: &SetArgs, ctx: &mut crate::CliContext) -> anyhow::Result<()> {
    ...
    let document = if args.wrapped {
        serde_json::from_str(&input).context("provided document is not valid JSON")?
    } else {
        let value = if args.json {
            serde_json::from_str(&input).context("value is not valid JSON")?
        } else {
...
async fn wrap_secret(
    ...
    serde_json::from_str(&body).context("config-encryption returned invalid JSON")
}
