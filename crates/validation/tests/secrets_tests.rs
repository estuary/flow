...
/// secrets named `<prefix>/<image repository>/<leaf>`, which is how a vendor's
/// OAuth client secret reaches the tasks of every tenant using that connector.
...
test://example/catalog.yaml:
  captures:
    acmeCo/source-widgets:
      secrets:
        vendorCo/oauth/source/image/oauth-client: /credentials/token
  collections:
    acmeCo/rollups:
      derive:
        secrets:
          vendorCo/oauth/derive/image/oauth-client: /credentials/user
  materializations:
    acmeCo/dest-widgets:
      secrets:
        vendorCo/oauth/dest/image/oauth-client: /credentials/user
"#,
    );
    ...
    acmeCo/rollups:
      derive:
        ...
        secrets:
          ...
          vendorCo/oauth/ghcr.io/estuary/derive-python/oauth-client: /environment/TOKEN
  materializations:
    acmeCo/dest-widgets:
      endpoint:
        connector: null
        dekaf:
          ...
          config: { token: null }
      secrets:
        vendorCo/oauth/ghcr.io/estuary/dekaf-kafka/oauth-client: /token
