...
pub fn run(fixture_yaml: &str, patch_yaml: &str) -> Outcome {
    ...
    for (capture, mock) in &mock_calls.live_captures {
        ...
        let resource_path_pointers = vec![
            doc::Pointer::from_str("/src-schema"),
            doc::Pointer::from_str("/src-name"),
        ];
        let bindings: Vec<flow::capture_spec::Binding> = mock
            ...
            .map(|binding| {
                let resource_path =
                    validation::extract_resource_path(&resource_path_pointers, &binding.resource)
                        .unwrap();
    ...
    for (materialization, mock) in &mock_calls.live_materializations {
        ...
        let resource_path_pointers = vec![
            doc::Pointer::from_str("/tgt-schema"),
            doc::Pointer::from_str("/tgt-name"),
        ];
                            ...
                            spec: Some(capture::response::Spec {
                                ...
                                resource_config_schema_json: serde_json::json!({
                                    ...
                                    "properties": {
                                        ...
                                        "src-name": {"type": "string"},
                                    },
                                    "required": ["source"]
                                })
                                .to_string(),
                                resource_path_pointers: vec![
                        ...
                        .yield_(materialize::Response {
                            spec: Some(materialize::response::Spec {
                                ...
                                resource_config_schema_json: serde_json::json!({
                                    "type": "object",
                                    "properties": {
                                        "tgt-schema": {"type": "string", "x-schema-name": true},
                                        "tgt-name": {"type": "string", "x-collection-name": true},
                                    },
                                    "required": ["tgt-name"]
                                })
                                .to_string(),
