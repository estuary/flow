...
fn extend_read_bundle(
    ...
    if read_bundle.get().contains(super::REF_WRITE_SCHEMA_PATTERN) {
        ...
        _ = write_schema.insert(
            ...
            models::RawValue::from_value(&Value::String(REF_WRITE_SCHEMA_URL.to_string())),
        );
        // Add as a definition within the read schema.
        read_defs.insert(
            REF_WRITE_SCHEMA_URL.to_string(),
    ...
    if let Some(inferred_bundle) = inferred_bundle {
        let mut inferred_schema: Skim = serde_json::from_str(inferred_bundle.get()).unwrap();

        // Set $id to "flow://inferred-schema".
        _ = inferred_schema.insert(
            keywords::ID.to_string(),
            models::RawValue::from_value(&Value::String(REF_INFERRED_SCHEMA_URL.to_string())),
        );
        // Add as a definition within the read schema.
        read_defs.insert(
            REF_INFERRED_SCHEMA_URL.to_string(),
...
const REF_INFERRED_SCHEMA_URL: &str = "flow://inferred-schema";
// URL for referencing the write schema of a collection, which may be used within a read schema.
const REF_WRITE_SCHEMA_URL: &str = "flow://write-schema";
