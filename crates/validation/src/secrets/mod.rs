...
pub fn walk_model(
    ...
    // secrets it actually declares, and each to the configuration location
    // declared for it, which publication cannot see.
    let task_prefix = parent_prefix(name).unwrap_or("");

    let mut config = ctx.config.to_value();

    for (secret, pointer) in ctx.secrets.iter() {
        let scope = scope.push_prop("secrets");
        let scope = scope.push_prop(secret);

        super::indexed::walk_name(scope, "secret", secret, models::Secret::regex(), errors);

        if let Err(err) = super::indexed::walk_ptr(pointer) {
            err.push(scope, errors);
            continue;
        }

        // Dry-run injection of resolved secrets to ensure the config is compatible.
        match json::ptr::create_value(&json::Pointer::from(pointer), &mut config) {
            Some(target) => *target = serde_json::Value::Null,
            None => Error::SecretsPointerIncompatible {
                entity,
                name: name.to_string(),
        ...
        // The reserved component makes the boundary between an arbitrary
        // catalog prefix and the exact image repository unambiguous.
        if ctx
            .image
            .as_ref()
            .is_some_and(|image| models::image_owns_secret(secret, image))
        {
            continue;
        }
