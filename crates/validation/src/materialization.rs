    ...
    for (index, (mut path, mut model, validate_validated)) in bindings.into_iter().enumerate() {
        ...
        match (
            ...
            (false, true, models::OnIncompatibleSchemaChange::Backfill) => {
                model_fixes.push(format!(
                    "backfilling binding due to incompatible field selection for collection {} (onIncompatibleSchemaChange: backfill)",
                    collection.name
                ));
                model.backfill += 1;
            ...
            (false, true, models::OnIncompatibleSchemaChange::DisableBinding) => {
                model_fixes.push(format!(
                    "disabling binding due to incompatible field selection for collection {} (onIncompatibleSchemaChange: disableBinding)",
                    collection.name
                ));
                model.disable = true;
            ...
            (false, true, models::OnIncompatibleSchemaChange::DisableTask) => {
                model_fixes.push(format!(
                    "disabling materialization due to incompatible field selection for collection {} (onIncompatibleSchemaChange: disableTask)",
                    collection.name
                ));
                shards.disable = true;
