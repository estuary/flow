...
pub enum Error {
    ...
    #[error(
        "{entity} {name} may use only secrets which are its siblings, or which its connector image vouches for, but secret {secret} is neither directly under the prefix {prefix} nor named {image}/<secret>, prefixed by the catalog prefix which owns it"
    )]
