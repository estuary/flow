...
pub enum Error {
    ...
    #[error(
        "{ref_entity} {ref_name}, referenced by {this_entity} {this_name}, was not found; did you mean {ref_entity} {suggest_name}?"
    )]
    NoSuchEntitySuggest {
        this_entity: &'static str,
        this_name: String,
    ...
    #[error(
        "could not map {this_entity} {this_name} into a storage mapping; did you mean {suggest_prefix}?"
    )]
    ...
    #[error(
        "{this_thing} references {ref_entity} {ref_name}, defined at {ref_scope}, without importing it or being imported by it"
    )]
    MissingImport {
        this_thing: String,
        ref_entity: &'static str,
    ...
    #[error(
        "collection {collection} must define either `schema` or both of `writeSchema` and `readSchema`"
    )]
    ...
    #[error(
        "the logical partitions of existing collection {collection} cannot change (from {live:?} to {draft:?}) without also resetting it"
    )]
    ...
    #[error(
        "location {ptr} accepts {type_:?} in schema {schema}, but locations used as keys may only be null-able integers, strings, or booleans"
    )]
    KeyWrongType {
        ptr: String,
        type_: types::Set,
    ...
    #[error(
        "groupBy field {field} accepts {type_:?}, but groupBy locations may only be null-able integers, strings, or booleans"
    )]
    GroupByWrongType { field: String, type_: types::Set },
    #[error(
        "the group-by key of non-delta resource {path:?} cannot change (from {live:?} to {draft:?}) without a backfill"
    )]
    GroupByChanged {
        path: Vec<String>,
        live: Vec<String>,
    ...
    #[error(
        "location {ptr} is {read_type:?} in readSchema {read_schema}, but {write_type:?} in writeSchema {write_schema}. Types of keyed locations must be the same in read and write schemas."
    )]
    ...
    #[error(
        "location {ptr} has a `reduce` strategy, which is disallowed because the location is used as a key"
    )]
    KeyHasReduce {
        ptr: String,
        schema: Url,
    ...
    #[error(
        "location {ptr} has a `redact` strategy, which is disallowed because the location is used as a key"
    )]
    KeyHasRedact {
        ptr: String,
        schema: Url,
    ...
    #[error(
        "projection {field} is the canonical field name of location {canonical_ptr:?}, and cannot re-map it to {wrong_ptr:?}"
    )]
    ...
    #[error(
        "{category} partition selector field {field} value {value} is incompatible with the projections type, {type_:?}"
    )]
    ...
    #[error(
        "transform {lhs_name} shuffled key types {lhs_types:?} don't align with transform {rhs_name} types {rhs_types:?}"
    )]
    ...
    #[error(
        "transform {name} shuffled key types {types:?} don't align with declared shuffle key types {given_types:?}"
    )]
    ...
    #[error(
        "this {entity} binding at offset {rhs_index} is bound to endpoint resource {path:?}, but so is the binding at offset {lhs_index}, which is not permitted (each binding must have a unique endpoint resource)"
    )]
    ...
    #[error(
        "current publication ID {pub_id} has been superseded by a larger last_pub_id {last_pub_id} of the spec; please retry the operation"
    )]
    ...
    #[error(
        "current build ID {build_id} has been superseded by a larger build ID {larger_id}; please retry the operation"
    )]
    BuildSuperseded {
        build_id: models::Id,
        larger_id: models::Id,
    ...
    #[error(
        "draft model is a 'touch' operation but a corresponding live model doesn't exist (and must)"
    )]
    TouchModelIsCreate,
    ...
    #[error(
        "{entity} {name} may use only secrets which are its siblings, or which its connector image vouches for, but secret {secret} is neither directly under the prefix {prefix} nor named <prefix>/connectors/{image}/<secret>"
    )]
    SecretNotSiblingOrImage {
        entity: &'static str,
        name: String,
        secret: String,
        prefix: String,
    ...
    #[error(
        "{entity} {name} cannot apply secret {secret} at endpoint configuration location {ptr}: the pointer is incompatible with the configuration's structure"
    )]
    SecretsPointerIncompatible {
        entity: &'static str,
        name: String,
        secret: String,
        ptr: String,
