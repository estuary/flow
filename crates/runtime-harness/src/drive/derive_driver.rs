...
where
    ...
    L: LoggerFactory,
{
    let join_shards = crate::drive::shards::build_derive_join_shards(run.n_shards, spec)?;

    // SQLite derivations require a VFS URI; the harness supplies a plain
    // tempfile path (the connector opens it with SQLite's default file VFS).
    let is_sqlite = spec
        .derivation
        .as_ref()
