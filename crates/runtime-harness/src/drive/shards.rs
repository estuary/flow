//! Synthetic shard topology for the local drive layer. The two shard lists
...
/// driver. `id`, `reactor`, and `etcd_create_revision` are synthesized;
...
/// Build the per-shard `proto::join::Shard` list used by the capture driver.
/// Captures are leaderless and have no shuffle topology, but each shard still
/// carries its own key-range slice and identity.
...
/// Build the per-shard `proto::join::Shard` list used by the derive driver.
/// Like materialize, derivations have a leader and a shuffle topology; the
/// synthetic split is on key only (full r-clock range).
