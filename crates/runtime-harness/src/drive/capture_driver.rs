//! Capture driver for the local drive layer.
//!
...
//! The driver mirrors that — N synthetic shards each driving one long-lived
...
where
    ...
    L: LoggerFactory,
{
    let join_shards = crate::drive::shards::build_capture_join_shards(run.n_shards, spec)?;
