...
use crate::drive::services::Run;
use anyhow::Context;
use prost::Message;
use proto_flow::{flow, runtime as cruntime};
use runtime_next::{LoggerFactory, PublisherFactory, proto};
use tokio::sync::mpsc;
use tokio_stream::wrappers::UnboundedReceiverStream;
use tokio_util::sync::CancellationToken;

/// Run sessions against the prepared topology. Sessions are numbered `1..` for
/// log context, and all run over the same per-shard SessionLoop streams.
pub async fn run_sessions<P, L>(
    run: &Run,
...
where
    ...
    L: LoggerFactory,
{
    let join_shards = crate::drive::shards::build_materialize_join_shards(run.n_shards, spec)?;
    ...
    L: LoggerFactory,
{
    ...
    if shard_index == 0 && !controls.initial_state_json.is_empty() {
        crate::drive::seed_rocksdb_state(
    ...
    for (idx, target_txns) in session_targets.into_iter().enumerate() {
        ...
        // A fixture / test run reads each session from its own directory (fresh
