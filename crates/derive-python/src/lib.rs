...
pub fn run() -> anyhow::Result<()> {
    ...
    tracing::info!(
        ...
        "starting Python derivation runtime"
    );
...
fn validate(validate: derive::request::Validate) -> anyhow::Result<derive::response::Validated> {
    ...
    if !config.module.chars().any(char::is_whitespace) {
        let stub_path = format!("{}/{}", project_root, config.module);
        generated_files.insert(
            stub_path.clone(),
        ...
        tracing::debug!(
            ...
            stub_path = %stub_path,
            "generated Python types, package files, and module stub"
        );

        // There's no further validation we can do without the actual module source.
