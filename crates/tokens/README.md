...
- A credential with a hard expiry passes its remaining lifetime through
  ...
  than once a minute) so that a failed refresh still has runway.
  `jwt::SignedSource` does this for its own signed tokens, and a `RestSource`
  implementor does it within its own `extract()`.
