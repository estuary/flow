...
/// Watch a Source via a spawned tokio task, refreshing on its stated cadence.
/// Returns a PendingWatch that's ready after its first refresh. The spawned
/// task stops when all clones of the Watch are dropped.
