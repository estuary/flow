...
pub enum Error {
    ...
    SchemaBuild(#[from] ::json::schema::build::Errors<annotation::Annotation>),
...
fn prepare_config_for_sops(
    ...
    let schema = json::schema::build::<annotation::Annotation>(&schema_url, schema)?;
    ...
    let (is_valid, outcomes) = validator.validate(&schema, &config, |outcome| {
        ...
        match &outcome {
            json::validator::Outcome::Annotation(annotation::Annotation::Secret(true)) => {
                Some(outcome)
            }
...
pub async fn encrypt_config(
    axum::Extension(default_keychain): axum::Extension<Arc<Keychain>>,
    axum::Json(Request {
        schema,
        config,
        keychain,
...
mod test {
    ...
    // Public key: age1z3nuz2xvzwrsjcrx0ewwa0cy0422m6dcunv87lzkg828j6whs5astjfuxg
    ...
    async fn test_invoke_sops_various_structures() {
        ...
        assert!(encrypted["string_sops"].as_str().unwrap().starts_with("ENC["));
        assert!(encrypted["number_sops"].as_str().unwrap().starts_with("ENC["));
        assert!(encrypted["bool_sops"].as_str().unwrap().starts_with("ENC["));
        assert!(encrypted["nested"]["password_sops"].as_str().unwrap().starts_with("ENC["));
        assert!(encrypted["nested"]["deep"]["api_key_sops"].as_str().unwrap().starts_with("ENC["));
        ...
        assert!(encrypted["array_of_objects"][0]["password_sops"]
            .as_str()
            .unwrap()
            .starts_with("ENC["));
        assert!(encrypted["array_of_objects"][1]["password_sops"]
            .as_str()
            .unwrap()
            .starts_with("ENC["));
