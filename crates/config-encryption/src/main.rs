...
async fn async_main(
    ...
    let default_keychain = match kms_type {
        KeychainType::Age => {
            config_encryption::Keychain::Age(kms_key);
        }
        KeychainType::Aws => {
            config_encryption::Keychain::Aws(kms_key);
        }
        KeychainType::Gcp => {
            config_encryption::Keychain::Gcp(kms_key);
        }
    };
    ...
    let api_listener = tokio::net::TcpListener::bind(format!("[::]:{port}"))
        .await
    ...
    let app = axum::Router::new()
        ...
        .layer(axum::Extension(Arc::new(default_keychain)))
        .layer(cors)
