...
pub fn combiner_perf() {
    ...
    let mut combiner = derive::combiner::Combiner::new(schema.curi.clone(), key.clone());
    ...
    for _round in 0..TOTAL_ROUNDS {
        ...
        combiner.combine_right(doc, &mut val).unwrap();
    }
    ...
    // Drain the combiner.
    for (_entry, _reduced) in combiner.drain_entries() {
        drained += 1;
    }
