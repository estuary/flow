    ...
    fn invoke(
        ...
        match (code, &mut self.pimpl) {
            ...
            (0, None) => {
                ...
                self.pimpl = Some(Pimpl::from_database(
                    env,
                    &cfg.catalog_path,
                    &cfg.local_dir,
                    &cfg.derivation,
...
impl<'e> Pimpl<'e> {
    ...
    fn from_database(
        ...
        pimpl.runtime.block_on(
            pimpl
                .node
                .as_ref()
                .unwrap()
                .invoke_bootstrap(pimpl.ctx.derivation_id),
        )?;
...
async fn process_block(
    ...
    let b_open_open = bytes::Bytes::from(&b"[["[..]);
    let b_close_close = bytes::Bytes::from(&b"]]"[..]);
    let b_close_comma_open = bytes::Bytes::from(&b"],["[..]);
    ...
    let mut tf_derived_docs = block
        ...
        .iter()
        .zip(source_columns.iter().cloned())
        .zip(register_columns.iter().cloned())
...
mod tests {
    ...
    fn test_basic_rpc() {
        ...
        let mut api = TestPimpl::new(&env);

        assert_eq!(api.pimpl.restore_checkpoint().unwrap(), Default::default());

        api.pimpl.begin_transaction().unwrap();
        ...
        let flush = api.pimpl.flush_transaction(
            derive_api::Flush {
        ...
        assert!(matches!(api.pimpl.state, State::BeginTxn(..)));
    }
    ...
    fn test_register_validation_error() {
        ...
        assert!(matches!(api.pimpl.state, State::Invalid));
    }
    ...
    impl<'e> TestPimpl<'e> {
        fn new(env: &'e rocksdb::Env) -> TestPimpl {
            ...
            let runtime = Pimpl::build_runtime();
            ...
            let pimpl =
                Pimpl::from_parts(runtime, schema_index, ctx, env, tmpdir.path(), None).unwrap();
        ...
        fn apply_documents(&mut self, documents: Vec<(i32, Value)>) {
            ...
            for (tf_id, doc) in documents {
                ...
                futures::executor::block_on(self.pimpl.doc_body(&w)).unwrap();
                w.clear();
            }
            ...
            self.pimpl
                ...
                .unwrap();
            futures::executor::block_on(self.pimpl.doc_body(b"garbage (not used)")).unwrap();
        }
    }
}
