...
///
/// `document` is one wrapped document exactly as config-encryption's
/// `/secret/encrypt` returned it. The control plane holds no grant on the KMS
/// key which wraps it, and so can neither read nor verify it: it is stored
/// verbatim, with its key order intact, because sops MACs traversal order.
#[derive(
    Debug, serde::Serialize, serde::Deserialize, schemars::JsonSchema, validator::Validate,
)]
#[serde(rename_all = "camelCase")]
pub struct TaskSetSecretRequest {
    /// # Catalog name of the secret to set.
    /// It must be a sibling of the task: a connector rotates only secrets the
    /// task itself owns, never one reached under the image rule.
    #[validate(nested)]
    pub name: crate::Name,
    /// # The sops-wrapped document to store.
    pub document: crate::RawValue,
    /// # JWT token signed by the requesting data-plane.
    /// It carries AUTHORIZE | TASK_UPDATE and names the task in its selector.
    pub token: String,
}
...
///
/// Both fields wholly replace their model counterparts and nothing else: this
/// is not a patch. The request does not publish. It records the update for the
/// task's controller, which publishes it and reports the outcome in status.
#[derive(
    Debug, serde::Serialize, serde::Deserialize, schemars::JsonSchema, validator::Validate,
)]
#[serde(rename_all = "camelCase")]
pub struct TaskUpdateConfigRequest {
    ...
    /// # JWT token signed by the requesting data-plane.
    /// It carries AUTHORIZE | TASK_UPDATE, names the task in its selector, and
