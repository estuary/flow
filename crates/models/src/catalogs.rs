...
mod tests {
    ...
    fn catalog_serialization() {
        ...
        resources.insert(
            "test://example/foo/bar.json".to_string(),
            ResourceDef {
        ...
        let import = vec![Import::Url(RelativeUrl::new("test://example/foo/bar.json"))];
        let catalog = Catalog {
            resources,
            import,
            ..Default::default()
        ...
        assert!(serde_json::from_str::<Catalog>(&serialized).is_ok());
        assert!(serde_yaml::from_str::<Catalog>(&serialized).is_ok());
    }
}
