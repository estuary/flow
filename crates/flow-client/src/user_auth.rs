...
impl proto_auth::TokenSource for UserTokenSource {
    ...
    async fn refresh(
        ...
        let access_token = if let Some(token) = &self.tokens.access_token {
            let unverified =
                proto_auth::jwt::parse_unverified::<serde::de::IgnoredAny>(token.as_bytes())?;
        ...
        let valid_for = match (access_token, &self.tokens.refresh_token) {
            ...
            (None, None) => std::time::Duration::MAX,
            ...
            (Some((access_token, valid_for)), Some(refresh_token))
                if valid_for > std::time::Duration::from_secs(60) =>
            {
                valid_for
            }
        ...
        Ok(Ok((self.tokens.clone(), valid_for)))
    }
}
...
pub async fn create_refresh_token(
    ...
    access_token: &str,
) -> tonic::Result<(RefreshToken, String, std::time::Duration)> {
...
pub async fn exchange_refresh_token(
    ...
    let unverified =
        proto_auth::jwt::parse_unverified::<serde::de::IgnoredAny>(access_token.as_bytes())?;
