...
pub fn infer(schema: JsValue) -> Result<JsValue, JsValue> {
    ...
    let parsed_schema = ::serde_wasm_bindgen::from_value(schema).map_err(|err| {
    ...
    // Skip the first property, since it's always just the root.
    let properties: Vec<Property> = shape
        ...
        .collect();
    serde_wasm_bindgen::to_value(&properties).map_err(|err| {
...
#[wasm_bindgen]
pub fn greet() {
    alert("Hello, flow-web!");
}

#[wasm_bindgen]
pub fn yeet() {
    let ptr = ::doc::Pointer::from_str("/foo/bar");
    let s = format!("ptr={:?}", ptr);
    alert(&s);
}
