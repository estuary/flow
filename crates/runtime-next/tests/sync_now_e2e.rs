...
/// Source documents reported by each scripted `Loaded`, and asserted back out
/// of sync-now `Status` / `Done` statistics.
...
impl Harness {
    ...
    /// SyncNow with a correctly-scoped READ token, returning the Ack and the
    /// still-open stream.
    async fn poke(
        &self,
        task_name: &str,
    ) -> (
        sync_now_response::Outcome,
        ...
        let mut stream = self
            ...
            .await
            .expect("SyncNow call");
        let (outcome, status) = expect_ack(&mut stream).await;
        (outcome, status, stream)
    }

    /// Drain barrier: poke until the Ack is `IDLE`. `Tail::Done` isn't
    /// directly observable, so this guarantees the next transaction starts
    /// from a fully-drained leader (deterministic `await_dones`) — and
    /// exercises idempotent re-poking for free. Only valid while no
    /// transaction is open (a non-idle poke arms `close_requested`).
    async fn poke_until_idle(&self, task_name: &str) {
        let deadline = tokio::time::Instant::now() + EXPECT_TIMEOUT;
        loop {
            let (outcome, status, mut stream) = self.poke(task_name).await;
            if outcome == sync_now_response::Outcome::Idle {
                assert_eq!(
                    status,
                    sync_now_response::Status::default(),
                    "IDLE Ack has zero-valued Status"
                );
                let done = expect_done(&mut stream).await;
                assert_eq!(
                    done,
                    sync_now_response::Done::default(),
                    "IDLE Done is zero-valued",
                );
                return;
            }
            assert!(
                tokio::time::Instant::now() < deadline,
                "timed out awaiting an IDLE ack (last outcome {outcome:?})",
            );
            // Early hangup is harmless; the prior transaction still drains.
            drop(stream);
...
async fn expect_ack(
    stream: &mut SyncNowStream,
) -> (sync_now_response::Outcome, sync_now_response::Status) {
    match recv_sync_now(stream, "Ack", EXPECT_TIMEOUT).await {
        sync_now_response::Response::Ack(ack) => (
            sync_now_response::Outcome::try_from(ack.outcome).expect("valid Outcome"),
            ack.status.unwrap_or_default(),
        ),
        other => panic!("expected SyncNow Ack, got {other:?}"),
    }
}

/// Await Done, skipping any interleaved Progress heartbeats.
async fn expect_done(stream: &mut SyncNowStream) -> sync_now_response::Done {
    loop {
        match recv_sync_now(stream, "Done", EXPECT_TIMEOUT).await {
            sync_now_response::Response::Done(done) => return done,
            sync_now_response::Response::Progress(_) => continue,
...
async fn expect_progress(
    ...
    match recv_sync_now(stream, "Progress", timeout).await {
        sync_now_response::Response::Progress(status) => status,
        other => panic!("expected SyncNow Progress, got {other:?}"),
    }
}
...
async fn warm_up_txn(harness: &Harness, driver: &mut ShardDriver, task_name: &str) {
    ...
    driver.send_acknowledged();
    harness.poke_until_idle(task_name).await;
}

/// Open a transaction that the sync schedule holds, and deliver the SyncNow
/// that collapses it, returning the caller's stream and its Ack Status.
///
/// Wall-clock flake guard: the schedule's fire instants sit on a fixed grid
/// (`jitter(seed) + k * baseInterval`) which the test cannot choose "now"
/// against, so a transaction could open milliseconds before a grid instant
/// and commit on its own (~1-in-1e5 odds with the 2h interval). If the Ack is
/// not HELD_COLLAPSED, drain that commit and retry with the next transaction,
/// whose fire instant is then a full interval away. One retry suffices;
/// flaking twice is a real failure.
async fn open_held_txn(
    harness: &Harness,
    driver: &mut ShardDriver,
) -> (SyncNowStream, sync_now_response::Status) {
    for attempt in 0..2 {
        harness.push_frontier();
        driver.expect_load().await;
        driver.send_loaded(DOCS_PER_TXN);

        let (outcome, status, mut stream) = harness.poke(SCHEDULED_TASK).await;
        if outcome == sync_now_response::Outcome::HeldCollapsed {
            return (stream, status);
        }
        assert_eq!(
            attempt, 0,
            "schedule hold flaked twice (outcome {outcome:?})",
        );
        driver.run_close_script().await;
        driver.send_acknowledged();
        expect_done(&mut stream).await;
        harness.poke_until_idle(SCHEDULED_TASK).await;
    }
...
/// the shard side acknowledges; a fully-drained task then acks IDLE.
...
async fn held_transaction_collapses_and_resolves_on_ack() {
    ...
    let (mut stream, ack_status) = open_held_txn(&harness, &mut driver).await;
    assert_eq!(ack_status.sourced_docs_total, DOCS_PER_TXN);
    assert_eq!(ack_status.sourced_bytes_total, DOCS_PER_TXN * 100);
    assert!(!ack_status.head_phase.is_empty(), "{ack_status:?}");
    assert!(!ack_status.tail_phase.is_empty(), "{ack_status:?}");
    ...
    driver.send_acknowledged();
    let done = expect_done(&mut stream).await;
    assert_eq!(done.committed_docs_total, DOCS_PER_TXN);
    assert_eq!(done.committed_bytes_total, DOCS_PER_TXN * 100);

    // With everything drained and no new frontier, a fresh poke is IDLE with
    // zero-valued Status and an immediate zero-valued Done.
    let (outcome, status, mut stream) = harness.poke(SCHEDULED_TASK).await;
    assert_eq!(outcome, sync_now_response::Outcome::Idle);
    assert_eq!(status, sync_now_response::Status::default());
    let done = expect_done(&mut stream).await;
    assert_eq!(done, sync_now_response::Done::default());
...
async fn concurrent_pokes_share_one_commit() {
    ...
    // the (single) commit is in flight. Depending on where the actor is when
    // each arrives, later pokes are HELD_COLLAPSED or ALREADY_CLOSING.
    let (first, _status) = open_held_txn(&harness, &mut driver).await;
    let mut streams = vec![first];
    for _ in 0..3 {
        let (outcome, _status, stream) = harness.poke(SCHEDULED_TASK).await;
        assert!(
            matches!(
                outcome,
                sync_now_response::Outcome::HeldCollapsed
                    | sync_now_response::Outcome::AlreadyClosing
            ),
            "unexpected concurrent-poke outcome {outcome:?}",
        );
        streams.push(stream);
    }
    ...
    driver.send_acknowledged();
    for stream in &mut streams {
        let done = expect_done(stream).await;
        assert_eq!(done.committed_docs_total, DOCS_PER_TXN);
    }
...
/// drains, acks ALREADY_CLOSING and resolves at the same acknowledgement
/// release.
#[tokio::test(flavor = "multi_thread")]
async fn poke_in_drain_window_is_already_closing() {
    let harness = Harness::start().await;
    let mut driver = harness.join_session(SCHEDULED_TASK).await;
    warm_up_txn(&harness, &mut driver, SCHEDULED_TASK).await;

    let (mut collapse_stream, _status) = open_held_txn(&harness, &mut driver).await;
    ...
    let (outcome, status, mut drain_stream) = harness.poke(SCHEDULED_TASK).await;
    assert_eq!(outcome, sync_now_response::Outcome::AlreadyClosing);
    assert_eq!(status.sourced_docs_total, DOCS_PER_TXN);

    driver.send_acknowledged();
    let done = expect_done(&mut collapse_stream).await;
    assert_eq!(done.committed_docs_total, DOCS_PER_TXN);
    let done = expect_done(&mut drain_stream).await;
    assert_eq!(done.committed_docs_total, DOCS_PER_TXN);
...
/// A transaction held by the 300s min-duration floor (no schedule) acks
/// CLOSE_REQUESTED and commits immediately — the close request bypasses the
/// floor. The floor holds even the session's first transaction
/// (`session_start` waives only schedule holds), so no warm-up is needed.
#[tokio::test(flavor = "multi_thread")]
async fn close_requested_on_min_duration_floor() {
    let harness = Harness::start().await;
    ...
    let (outcome, status, mut stream) = harness.poke(FLOOR_TASK).await;
    assert_eq!(outcome, sync_now_response::Outcome::CloseRequested);
    assert_eq!(status.sourced_docs_total, DOCS_PER_TXN);
...
/// A parked waiter receives Progress heartbeats. The cadence is the
/// production 15s `SYNC_NOW_HEARTBEAT` constant, kept un-injectable on
/// purpose, so this is deliberately the one slow test: the first beat lands
/// one full interval after the waiter parks.
#[tokio::test(flavor = "multi_thread")]
async fn parked_waiter_receives_progress_heartbeats() {
    let harness = Harness::start().await;
    let mut driver = harness.join_session(FLOOR_TASK).await;
    ...
    let (outcome, _status, mut stream) = harness.poke(FLOOR_TASK).await;
    assert_eq!(outcome, sync_now_response::Outcome::CloseRequested);

    // Commit, then park with the acknowledgement withheld.
    driver.run_close_script().await;

    // One Progress proves the machinery; don't wait for a second beat.
    let status = expect_progress(&mut stream, Duration::from_secs(30)).await;
    assert_eq!(status.sourced_docs_total, DOCS_PER_TXN);
    assert!(!status.head_phase.is_empty(), "{status:?}");
    assert!(!status.tail_phase.is_empty(), "{status:?}");
...
/// NOT_APPLICABLE resolution from the caller's shard-scope when no live
/// session matches.
#[tokio::test(flavor = "multi_thread")]
async fn authz_and_task_resolution() {
    ...
    // A correctly-scoped READ token is accepted (IDLE: no open transaction).
    harness.poke_until_idle(SCHEDULED_TASK).await;
    ...
    // Materialize leader sessions, and NOT_APPLICABLE is resolved by the
    // reactor front door, which has the shard keyspace to know a task's type.
    let err = harness
        .sync_now(
