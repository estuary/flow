...
pub async fn dial_and_join(
    ...
    let verify = crate::verify("Materialize", "Joined", &leader_endpoint, 0);
    ...
    R: futures::Stream<Item = tonic::Result<proto::Materialize>> + Send + Unpin + 'static,
{
    ...
    let verify = crate::verify("Materialize", "Open", "controller", 0);
    ...
    let mut binding_state_keys: Vec<(String, u32)> = task
        .bindings
        .iter()
        .enumerate()
        .map(|(i, b)| (b.state_key.clone(), i as u32))
        .collect();
    binding_state_keys.sort();
    ...
    let open = loop {
        let verify = crate::verify("Materialize", "Apply, Persist, or Open", "leader", 0);
        match verify.not_eof(leader_rx.next().await)? {
            ...
            proto::Materialize {
                ...
                crate::rocksdb::extend_write_batch(&mut wb, &persist, &task.binding_state_keys)
                    .context("failed to apply Persist")?;

                let mut wo = rocksdb::WriteOptions::new();
                wo.set_sync(true);
                db = db.write_opt(wb, wo).await.context("Persist failed")?;
    ...
    let open = materialize::request::Open {
        materialization: Some(spec),
        version,
        state_json: connector_state_json,
        range,
    };
    let (connector_tx, mut connector_rx, container) = super::connector::start(
        service,
        materialize::Request {
            open: Some(open),
            ..Default::default()
    ...
    let verify = crate::verify("Materialize", "Opened", "connector", 0);
    ...
    // Tell Controller of C:Opened, which completes startup.
