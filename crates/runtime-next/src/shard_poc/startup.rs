...
use super::recovery::{labels_build_for, recover_stream_from_recover, reduce_state_patches};
use crate::proto;
...
async fn stream_recover(
    ...
    let (rocksdb, recover) = match (rocksdb, is_shard_zero) {
        (Some(db), true) => {
            let (db, recover) = db
        ...
        (db, _) => (db, proto::Recover::default()),
    };

    for recover in recover_stream_from_recover(recover).context("building Recover stream")? {
        leader_tx
            .send(proto::Materialize {
        ...
        match msg {
            proto::Materialize {
                ...
                if !applied_patches.is_empty() {
                    let mut patches =
                        Vec::with_capacity(connector_patches.len() + applied_patches.len());
                    patches.extend_from_slice(&connector_patches);
                    crate::recovery::codec::extend_state_patches(&mut patches, &applied_patches);
                    connector_patches = Bytes::from(patches);
                }
...
async fn recv_recovered(
    ...
    loop {
        ...
        if !recovered.connector_patches_json.is_empty() {
            let mut out =
                Vec::with_capacity(patches.len() + recovered.connector_patches_json.len());
            out.extend_from_slice(&patches);
            crate::recovery::codec::extend_state_patches(
                &mut out,
                &recovered.connector_patches_json,
            );
            patches = Bytes::from(out);
        }
...
async fn forward_opened(
    ...
    // Only shard zero reports a connector checkpoint to the leader. The
    // controller still gets the local connector metadata for observability.
    let leader_checkpoint = is_shard_zero.then(|| runtime_checkpoint.clone()).flatten();
    leader_tx
        .send(proto::Materialize {
            opened: Some(proto::materialize::Opened {
                container: None,
                connector_checkpoint: leader_checkpoint,
            }),
            ..Default::default()
        })
        .await
