...
{
    let handler = service.registry.register("leader.derive");
...
where
    R: futures::Stream<Item = tonic::Result<proto::Derive>> + Send + Unpin + 'static,
{
    let verify = crate::verify("Derive", "Join", "shard");
    ...
    let outcome = {
        let mut guard = service.derive_joins.lock().unwrap();
