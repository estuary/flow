...
| `session` | `DerivationSession`: one derivation resident for the whole run, one transaction per `read()`. |
...
`tests/catalog_tests.rs` drives the whole path from inline YAML. Both use
derive-sqlite, which runs in-process — no Docker. Multi-shard *image*
...
**Waiting on a signal alone can only hang.** Neither `commit_rx` nor `reset_rx`
closes when a session dies (the tonic server outlives sessions and holds the
commit sender; reset senders are per-shard). `session::await_signal` therefore
selects, biased, against a `failure_rx` that every shard drainer reports into as
it exits — and a *closed* `failure_rx` is itself terminal. Drainers also log the
death through the task's ops logger, because the runtime reports a dead session
as a stream status, not as an ops log, so nothing else would land it in a
publication's job logs.
...
awaits one `ResetDone` each — safe because the session is quiescent between
...
**Every exit must pass through `shutdown_all`.** A resident session's shard
tasks hold RocksDB open inside the run's tempdir; dropping sessions without
stopping them lets the tempdir vanish under a live RocksDB and **aborts the
process**. `start_sessions` shuts down what it already started on a mid-loop
failure for the same reason, and `stop_shards` deliberately keeps `request_txs`
alive until every drainer confirms — EOF mid-handshake is rejected as an
unexpected controller EOF.

**Shard count is chosen by connector authority, not by the caller alone.**
derive-sqlite is remote-authoritative (its checkpoint lives in the endpoint) and
*must* be single-shard, so `run` forces `n_shards = 1` for it and ignores
`Options::splits`; image derivations get `splits` shards to exercise multi-shard
key routing. Starting a multi-shard derive-sqlite session is a hard error from
the leader, after the shards have already Opened —
`derive_sqlite::start_failure_after_open_reports_and_tears_down` guards that it
