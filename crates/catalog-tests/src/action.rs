...
pub async fn run_test_case<D: Driver>(
    ...
    loop {
        let (ready, next_ready, next_name) = graph.pop_ready_stats();
        ...
        // Advance time to unblock the next pending stat. V1 tolerated an
        // `ErrAdvanceDisabled` here, for a data plane not serving the Testing
        // RPC; synthetic time is entirely local to us, so advance cannot fail
        // that way.
        if let Some(next_ready) = next_ready {
            tracing::trace!(
                delta = %next_ready,
                task = next_name.as_deref().unwrap_or_default(),
