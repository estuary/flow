...
impl Graph {
    ...
    fn project_write(&mut self, collection: &str, write_at: &Clock) {
        ...
        for r in readers {
            ...
            // An absent entry is an empty clock, which contains only an empty
            // `read_through` — so a write projecting no journals of this
            // collection enqueues nothing, as in V1.
            let existing = self.read_through.get(&r.task).cloned().unwrap_or_default();
