//! `run_tests`: the crate's entry point, ported from `go/flowctl-go/cmd-api-test.go`.
//!
...
//! resetting connector state between cases via the runtime-next Reset flow. V1
//! resets after every case, including the last, and continues past a failing case
//! rather than aborting the run; both are matched here.
