...
use proto_flow::flow::{CollectionSpec, TestSpec};
...
impl Default for Options {
    fn default() -> Self {
        Self {
            connector_router: runtime_local::local_test_router(),
            splits: 2,
...
/// Dropping the returned future mid-run is safe.
///
...
/// concurrently (see [`MAX_CONCURRENT_STARTS`]).
async fn start_sessions(
    ...
    let n_shards = options.splits.max(1);

    // Select and validate up front, so a malformed catalog fails before any
    // connector boots, and so each start below closes over *owned* values. The
    // latter is load-bearing: a closure that's higher-ranked over the borrows in
    // `collections` defeats rustc's auto-trait inference in a caller which
    // awaits `run_tests` from deep within its own dispatch, and flowctl's CLI
    // future then fails to prove it is Send.
    ...
    for (name, spec) in collections {
        ...
        pending.push((name.clone(), spec.clone(), logger_factory));
    }
    ...
    let mut starts = futures::stream::iter(starts).buffer_unordered(MAX_CONCURRENT_STARTS);
    let mut sessions = BTreeMap::new();
    let mut errors = Vec::new();

    // Start all sessions, gathering startup errors for consolidated reporting.
    while let Some(started) = starts.next().await {
        match started {
            Ok((name, session)) => {
    ...
    Err(anyhow::anyhow!(
        "{} derivation sessions failed to start:\n{}",
        errors.len(),
        errors
            .into_iter()
            .map(|err| format!("{err:#}"))
...
/// `use<>`: the factory clones what it needs and borrows neither argument. Under
/// edition 2024 an unqualified `impl Trait` captures every in-scope lifetime, and
/// a factory carrying two phantom lifetimes makes the futures holding it
/// higher-ranked — which defeats auto-trait inference in callers that await
/// `run_tests` from deep within their own dispatch, down to flowctl's CLI future
/// failing to prove it is Send.
fn logger_factory(
    log_handler: &LogHandler,
    shard_template: &proto_gazette::consumer::ShardSpec,
) -> anyhow::Result<impl runtime_next::LoggerFactory + use<>> {
