...
pub struct Frontier {
    ...
    /// `journal_read_suffix`. Folded from immediately-committed CONTROL
    /// documents; does not participate in causal-hint sequencing.
    pub latest_backfill_begin: BTreeMap<String, Clock>,
    /// Latest committed backfill-complete clock of the checkpoint delta, keyed
    /// by `journal_read_suffix`. See `latest_backfill_begin`.
...
impl Frontier {
    ...
    /// Per-suffix max of two backfill clock maps. Used to fold a checkpoint
    /// delta's backfill metadata into the cumulative base during `reduce`.
    ...
    pub fn encode(&self) -> shuffle::Frontier {
        ...
        proto.latest_backfill_begin = self
            .latest_backfill_begin
            .iter()
            .map(|(suffix, clock)| shuffle::frontier::BackfillBegin {
        ...
        proto.latest_backfill_complete = self
            .latest_backfill_complete
            .iter()
            .map(|(suffix, clock)| shuffle::frontier::BackfillComplete {
                journal_read_suffix: suffix.clone(),
                clock: clock.as_u64(),
            })
            .collect();
...
mod test {
    ...
    fn test_frontier_reduce() {
        ...
        // Per-suffix max of backfill clocks across both inputs.
        assert_eq!(
