...
/// Keys are unique because Frontier journals are unique on (journal, binding),
...
fn build_visibility_index(
    ...
    debug_assert!(
        frontier
            .journals
            .windows(2)
            .all(|w| (&w[0].journal, w[0].binding) < (&w[1].journal, w[1].binding))
    );

    // Block journals are sorted by name only (no binding dimension).
    // Frontier is sorted by (journal, binding), so journal names advance
    // monotonically (with possible repeats for multiple bindings).
    ...
    for jf in &frontier.journals {
        // Advance cursor to find matching journal name. Under (journal, binding)
        // ordering, names advance monotonically with possible repeats for
        // multiple bindings — the cursor naturally stays at the right position.
