...
fn try_compress(path: &std::path::Path) -> anyhow::Result<u64> {
    ...
    let mut tmp_file = tempfile::NamedTempFile::with_prefix_in(
        format!(".compress-{}", filename.to_string_lossy()),
        dir,
    )?;
    ...
    rename_exchange(tmp_file.path(), path).with_context(|| {
        format!(
            "atomic rename of compressed segment {:?} -> {path:?}",
            tmp_file.path(),
        )
    })?;
    drop(tmp_file); // Delete the uncompressed original.
