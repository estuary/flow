...
/// Connection scheme parsed from broker URL.
...
/// Creates a length-delimited codec for Kafka protocol framing.
...
async fn async_connect(broker_url: &str) -> anyhow::Result<BoxedKafkaConnection> {
    ...
    let stream: MaybeTlsStream = match scheme {
        ConnectionScheme::Plaintext => {
            tracing::debug!(port, host = ?host, "Plaintext connection established");
        ...
        ConnectionScheme::Tls => {
            ...
            tracing::debug!(port, host = ?host, "TLS connection established");
...
impl KafkaApiClient {
    ...
    async fn try_connect(url: &str, mut auth: KafkaClientAuth) -> anyhow::Result<Self> {
        ...
        // Optionally perform SASL authentication if auth is configured.
    ...
    #[instrument(skip_all)]
    async fn try_increase_partition_counts_once(
        coord: &mut KafkaApiClient,
        topics: &[(messages::TopicName, usize)],
    ) -> anyhow::Result<(Vec<(messages::TopicName, usize)>, Option<(i16, String)>)> {
        ...
        let mut retriable_error: Option<(i16, String)> = None;
        ...
        Ok((topics_to_retry, retriable_error))
    }
    ...
    async fn increase_partition_counts(
        ...
        loop {
            let (topics_to_retry, retriable_error) =
                Self::try_increase_partition_counts_once(coord, &topics).await?;

            if !topics_to_retry.is_empty() {
                attempts += 1;
                if attempts > 3 {
                    let error_desc = retriable_error
                        .map(|(code, name)| format!("error {} ({}) ", code, name))
                        .unwrap_or_else(|| "transient errors ".to_string());
                    return Err(anyhow::anyhow!(
                        "Failed to increase partition count for topics {:?} due to {}after {} attempt(s)",
                        topics_to_retry,
                        error_desc,
                        attempts
                    ));
                }
                let sleep_secs = attempts.pow(2);
                tracing::warn!(
                    "Retrying topics {:?} after {}s (attempt {}/3)...",
                    topics_to_retry
                        .iter()
                        .map(|(n, _)| n.as_str())
                        .collect::<Vec<_>>(),
...
impl KafkaClientAuth {
    /// Create a `KafkaClientAuth` from an optional MSK region.
    ///
    /// - If `msk_region` is `Some`, creates MSK IAM authentication for that region.
    /// - If `msk_region` is `None`, creates no-auth mode for local testing.
    pub async fn from_msk_region(msk_region: Option<&str>) -> Self {
        match msk_region {
            Some(region) => {
