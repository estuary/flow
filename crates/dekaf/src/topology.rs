    ...
    pub async fn new(
        ...
        let binding = match auth {
            SessionAuthentication::Task(task_auth) => {
                task_auth.get_binding_for_topic(topic_name).await?
            }
            SessionAuthentication::Redirect { spec, .. } => {
        ...
        .context(format!("no binding found for topic '{topic_name}'"))?;
        ...
        let not_before = binding.not_before.map(|b| {
            uuid::Clock::from_unix(b.seconds.try_into().unwrap(), b.nanos.try_into().unwrap())
        });
        let not_after = binding.not_after.map(|b| {
            uuid::Clock::from_unix(b.seconds.try_into().unwrap(), b.nanos.try_into().unwrap())
        });
