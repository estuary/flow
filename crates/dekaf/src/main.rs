...
use dekaf::{
    KafkaApiClient, Session, TaskManager, UpstreamKafkaAuth, log_appender::GazetteWriter, logging,
};
use flow_client::{
...
impl UpstreamAuthArgs {
    fn build(&self) -> anyhow::Result<UpstreamKafkaAuth> {
        match self.upstream_auth {
            UpstreamAuth::Msk => {
                ...
                Ok(UpstreamKafkaAuth::Msk { region })
            }
            UpstreamAuth::None => Ok(UpstreamKafkaAuth::None),
        }
    }
}
...
async fn async_main(cli: Cli) -> anyhow::Result<()> {
    ...
    let upstream_auth = cli.upstream_auth.build()?;
...
async fn test_kafka(cli: &Cli) -> anyhow::Result<()> {
    let upstream_auth = cli.upstream_auth.build()?;
    let auth = upstream_auth.build_kafka_client_auth().await;
