...
// This makes TaskManagerError itself a valid error type.
...
pub struct TaskManager {
    ...
    tasks: std::sync::Mutex<
        HashMap<String, std::sync::Weak<watch::Receiver<Option<Result<TaskState>>>>>,
    >,
    interval: Duration,
...
impl TaskManager {
    ...
    async fn run_task_manager(
        ...
        receiver: Weak<watch::Receiver<Option<Result<TaskState>>>>,
        ...
        loop {
            ...
            if Weak::strong_count(&receiver) == 0 && timeout_start.is_none() {
...
) -> anyhow::Result<HashMap<String, Result<(journal::Client, proto_gazette::Claims, Vec<Partition>)>>>
{
    ...
    for binding in &spec.bindings {
        ...
        // Fetch a new client either if we don't have one or if the last attempt errored
