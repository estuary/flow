...
use crate::{
    DekafError, KafkaApiClient, SessionAuthentication, TaskState, UpstreamKafkaAuth,
...
pub struct Session {
    ...
    upstream_auth: UpstreamKafkaAuth,
...
impl Session {
    ...
    async fn get_kafka_client(&mut self) -> anyhow::Result<&mut KafkaApiClient> {
        if let Some(ref mut client) = self.client {
            ...
            let auth = self.upstream_auth.build_kafka_client_auth().await;
            ...
            for topic in consumer_protocol_subscription_msg.topics.iter_mut() {
                    ...
                    .map(|(_, status)| status)
                {
                    ...
                    Some(CollectionStatus::NotReady) => {
                        tracing::warn!(
                            topic = ?topic,
                            "JoinGroup: Collection exists but has no journals available"
                        );
                        return Ok(messages::JoinGroupResponse::default()
                            .with_error_code(ResponseError::LeaderNotAvailable.code()));
                    }
                    Some(CollectionStatus::NotFound) | None => {
                        tracing::warn!(topic = ?topic, "JoinGroup: Collection not found");
                        return Ok(messages::JoinGroupResponse::default()
                            .with_error_code(ResponseError::UnknownTopicOrPartition.code()));
                    }
                };
            ...
            for part in &consumer_protocol_assignment_msg.assigned_partitions {
                    ...
                    .map(|(_, status)| status)
                {
                    ...
                    Some(CollectionStatus::NotReady) => {
                        tracing::warn!(
                            topic = ?part.topic,
                            "SyncGroup: Collection exists but has no journals available"
                        );
                        return Ok(messages::SyncGroupResponse::default()
                            .with_error_code(ResponseError::LeaderNotAvailable.code()));
                    }
                    Some(CollectionStatus::NotFound) | None => {
                        tracing::warn!(topic = ?part.topic, "SyncGroup: Collection not found");
                        return Ok(messages::SyncGroupResponse::default()
                            .with_error_code(ResponseError::UnknownTopicOrPartition.code()));
                    }
                }
        ...
        let resp: anyhow::Result<_> = async {
            ...
            for (topic_name, status) in &collection_statuses {
                let error_code = match status {
                    ...
                    CollectionStatus::NotFound => {
                        tracing::warn!(topic = ?topic_name, "OffsetCommit: Collection not found");
                        ResponseError::UnknownTopicOrPartition.code()
                    }
                    CollectionStatus::NotReady => {
                        tracing::warn!(
                            topic = ?topic_name,
                            "OffsetCommit: Collection exists but has no journals available"
                        );
            ...
            for topic in &mut req.topics {
                // Look up collection to get backfill counter (all collections are Ready at this point)
                let backfill_counter = collection_statuses
                    .iter()
        ...
        if let Some(ref statuses) = collection_statuses {
            for (topic_name, status) in statuses {
                let error_code = match status {
                    CollectionStatus::Ready(_) => continue,
                    CollectionStatus::NotFound => {
                        tracing::warn!(topic = ?topic_name, "OffsetFetch: Collection not found");
                        ResponseError::UnknownTopicOrPartition.code()
                    }
                    CollectionStatus::NotReady => {
                        tracing::warn!(
                            topic = ?topic_name,
                            "OffsetFetch: Collection exists but has no journals available"
                        );
                        ResponseError::LeaderNotAvailable.code()
                    }
                };
