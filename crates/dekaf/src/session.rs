...
impl Session {
        ...
        let topics: anyhow::Result<_> = async {
            ...
            requests
                ...
                .map(|(request, (_, status))| {
                    ...
                    match status.ready() {
                        Ok(collection) => self.build_topic_metadata(topic_name, &collection),
                        Err(reason) => Ok(MetadataResponseTopic::default()
            ...
            for partition_request in &topic_request.partitions {
                ...
                let Some(pending) = self.reads.get_mut(&key) else {
                    ...
                    match Collection::new(&auth, &key.0).await {
                        ...
                        Ok(CollectionStatus::Unavailable(CollectionUnavailable::NotReady)) => {
                    ...
                    .map(|(_, status)| status)
                {
                    ...
                    other => {
                        let error_code = other
                            ...
                            .unwrap_or_else(|| ResponseError::UnknownTopicOrPartition.code());
                        tracing::warn!(topic = ?topic, error_code, "Collection not available");
                        return Ok(
                            messages::JoinGroupResponse::default().with_error_code(error_code)
                        );
                    }
                };
    ...
    pub async fn sync_group(
        ...
        for assignment in mutable_req.assignments.iter_mut() {
            ...
            for part in &consumer_protocol_assignment_msg.assigned_partitions {
                ...
                if !matches!(status, Some(CollectionStatus::Ready(_))) {
                    let error_code = status
                        .and_then(|s| s.error_code())
                        .unwrap_or_else(|| ResponseError::UnknownTopicOrPartition.code());
                    tracing::warn!(topic = ?part.topic, error_code, "Collection not available");
                    return Ok(messages::SyncGroupResponse::default().with_error_code(error_code));
                }
            }
    ...
    pub async fn offset_commit(
        ...
        let resp: anyhow::Result<_> = async {
            ...
            let TaskToken::Authorized { token, .. } = &*guard else {
    ...
    async fn is_fetch_data_preview(
        ...
        let result: anyhow::Result<_> = async {
            ...
            let collection = match Collection::new(auth, collection_name.as_str())
                .await?
                .ready()
            {
                Ok(c) => c,
