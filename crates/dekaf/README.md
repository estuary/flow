...
main.rs        CLI, listeners, and the process-wide task::Registry
  ...
  |
  +- task.rs      Shared, self-refreshing state of one task.
  |                 |
  |                 +- topology.rs  A binding assembled as a Collection:
  |                 |               Avro schemas, extractors, partitions.
...
Everything a session needs is held by [`task::Task`] as a tree of `tokens`
