        ...
        match node.as_node() {
            Node::Array(arr) => {
                ...
                let items = BumpVec::with_contents(
                    alloc,
                    arr.iter().map(|item| {
                        let (item, tape_delta) = Self::from_node_with_length(item, alloc);
                        tape_length += tape_delta;
                        item
                    }),
                );
                (HeapNode::Array(tape_length, items), tape_length)
            }
            ...
            Node::Object(fields) => {
                ...
                let fields = BumpVec::with_contents(
                    ...
                    fields.iter().map(|field| {
                        let (value, tape_delta) = Self::from_node_with_length(field.value(), alloc);
                        tape_length += tape_delta;
                ...
                (HeapNode::Object(tape_length, fields), tape_length)
            }
