...
impl Pointer {
    ...
    pub fn create_heap_node<'alloc>(
        ...
        let (matched, mut tape_delta) = loop {
            let Some((token, new_tail)) = tail.split_first() else {
                // Base case: replace `node` with `value`.
                let tape_delta = value.tape_length() - node.tape_length();
                *node = value;
                break (true, tape_delta);
            };
            tail = new_tail;
        ...
        for (tape_length, local_delta) in stack.into_iter().rev() {
            tape_delta += local_delta;
            *tape_length += tape_delta;
        }

        matched.then_some(tape_delta).ok_or(tape_delta)
    }
}
    ...
    fn test_ptr_create() {
        ...
        .iter_mut()
        {
            ...
            let tape_delta = ptr
                ...
                .unwrap();
            assert_eq!(tape_delta, *expect_delta);
