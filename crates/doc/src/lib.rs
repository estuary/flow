...
// This crate has two implementations of AsNode: a mutable HeapNode,
// and an ArchivedNode serialized by the `rkyv` crate.
// The serde_json::Value implementation is in the json crate.
mod archived;
...
mod test {
    ...
    fn test_sizes() {
        ...
        // Node is 24 bytes.
        assert_eq!(std::mem::size_of::<json::Node<'static, HeapNode<'static>>>(), 24);
        assert_eq!(std::mem::align_of::<json::Node<'static, HeapNode<'static>>>(), 8);
    }
}
