...
impl Shape {
    ...
    fn inspect_inner(
        ...
        // Is the user trying to block a location which must exist?
        ...
        // Is the user trying to hash a valid location which cannot be a string?
        // Suppress on invalid locations, as the user cannot otherwise apply an
        // annotation to a location which will be inferred, but hasn't yet.
        if matches!(self.redact, Redact::Strategy(redact::Strategy::Sha256))
            && self.type_ != types::INVALID
