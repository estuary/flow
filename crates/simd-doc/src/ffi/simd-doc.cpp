...
void Parser::parse(const rust::Slice<const uint8_t> input, int64_t offset, const Allocator &alloc, HeapNode &node, Parsed &output)
{
...
void Parser::transcode(const rust::Slice<const uint8_t> input, Transcoded &output)
{
    dom::document_stream stream = parser.parse_many(input.data(), input.size(), input.size());
