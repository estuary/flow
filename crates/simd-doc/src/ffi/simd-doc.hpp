...
// Returns a pair of (pnode, tape_length) where tape_length is the total number of nodes.
...
__attribute__((flatten)) std::pair<pnode, int32_t> transcode_array(pbuffer &buf, dom::array arr)
{
    ...
    int32_t tape_length = 1; // Initially 1 for this node.
    for (dom::element cur : arr)
    {
        auto [node, tape_delta] = transcode_node(buf, cur.type(), cur);
        *(item++) = node;
        tape_length += tape_delta;
    }
    pnode ret = place_array(buf, reinterpret_cast<pnode *>(scratch.data()), len, tape_length);

    buf.pool.emplace_back(std::move(scratch));
    return {ret, tape_length};
}
...
__attribute__((flatten)) std::pair<pnode, int32_t> transcode_object(pbuffer &buf, dom::object obj)
{
    ...
    for (dom::key_value_pair cur : obj)
    {
        ...
        auto [node, tape_delta] = transcode_node(buf, cur_type, cur.value);
        field->node = node;
        tape_length += tape_delta;
        field++;
    }
    ...
    pnode ret = place_object(buf, reinterpret_cast<pfield *>(scratch.data()), len, tape_length);
...
// Returns the tape_length of the parsed node.
int32_t parse_node(const Allocator &alloc, dom::element_type typ, dom::element elem, HeapNode &out)
{
    ...
    case dom::element_type::ARRAY:
    {
        ...
        int32_t *tape_length;
        set_array(alloc, out, arr.size(), items, tape_length);
        rust::Slice<HeapNode>::iterator it = items.begin();

        for (dom::element cur : arr)
        {
            *tape_length += parse_node(alloc, cur.type(), cur, *(it++));
        }
        return *tape_length;
    }
    case dom::element_type::OBJECT:
    {
        ...
        int32_t *tape_length;
        set_object(alloc, out, obj.size(), fields, tape_length);
        ...
        for (dom::key_value_pair cur : obj)
        {
            ...
            *tape_length += parse_node(alloc, cur_type, cur.value, child);
        }
