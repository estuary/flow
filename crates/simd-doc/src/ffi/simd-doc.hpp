...
// Forward declaration of walk_node, used by walk_array and walk_object.
pnode walk_node(pbuffer &buf, dom::element_type typ, dom::element elem);

// Encode a simdjson array using a depth-first walk of its items.
__attribute__((flatten)) pnode walk_array(pbuffer &buf, dom::array arr)
{
    ...
    {
        *(item++) = walk_node(buf, cur.type(), cur);
    }
...
// Sort the fields of an object.
__attribute__((noinline)) void sort_object(pbuffer &buf, pfield *const d, const uint64_t len)
{
...
// Encode a simdjson object using a depth-first walk of its fields.
__attribute__((flatten)) pnode walk_object(pbuffer &buf, dom::object obj)
{
    ...
    for (dom::key_value_pair cur : obj)
    {
        ...
        field->node = walk_node(buf, cur_type, cur.value);
        field++;
    }
    ...
    if (unsorted != 0)
    {
        sort_object(buf, reinterpret_cast<pfield *>(scratch.data()), len);
    }
...
inline pnode walk_node(pbuffer &buf, dom::element_type typ, dom::element elem)
{
    switch (typ)
    {
    case dom::element_type::ARRAY:
    {
        return walk_array(buf, elem.get_array());
    }
    case dom::element_type::OBJECT:
    {
        return walk_object(buf, elem.get_object());
    }
    ...
    case dom::element_type::STRING:
    {
        ...
        {
            // Store using inline representation.
            // SAFETY: See comment in walk_object().
            uint64_t word;
            memcpy(&word, d, 8);
