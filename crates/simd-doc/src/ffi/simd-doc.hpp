...
inline size_t decode_inline_str_length(uint64_t value)
{
    ...
    // Here, we add one to overflow all 0xFF padding to zero,
    // and then count the number of trailing zero bytes to determine
    // the number of non-padding bytes in the string.
    return 8 - (std::countr_zero(value + 1u) / 8);
}
