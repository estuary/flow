...
impl super::Parser {
    pub fn parse_simd<'a>(
        ...
        let consumed = ffi::parse_many(alloc, docs, &mut *input, &mut node, &mut self.0)?;
...
#[repr(transparent)]
pub struct Allocator(doc::Allocator);
#[repr(transparent)]
pub struct Docs<'a>(Vec<(usize, doc::HeapNode<'a>)>);
#[repr(transparent)]
pub struct HeapNode<'a>(doc::HeapNode<'a>);
#[repr(transparent)]
pub struct HeapField<'a>(doc::HeapField<'a>);
...
fn set_field<'a>(
    ...
    // Safety: UTF-8 has already been validated by simdjson.
