...
pub fn full_range_shards(count: u32) -> Vec<shuffle::proto::Shard> {
    // The ID prefix is irrelevant here: routing reads only `range`, and this
    // list is never joined against the leader's (which carries the real IDs).
    labels::shard::even_splits("", count, 1)
        .into_iter()
