...
//!
//! For materializations and derivations: one tonic server hosting
//! `runtime_next::Service` — plus, optionally, a caller-supplied
//! `shuffle::Service` for runs that read real journals — on a single ephemeral
//! 127.0.0.1 port, alongside a tempdir for shuffle log segments. The caller
//! supplies the
//! [`ShuffleSessionFactory`](runtime_next::ShuffleSessionFactory), so this layer
//! is unaware of where source documents come from. Drivers keep SessionLoop
//! streams open across sessions so RocksDB is reused without closing.
//!
//! For captures: just (optionally) the admin surface. Captures are leaderless
//! and don't read journals, so the in-process Leader / Shuffle services are not
//! constructed.
//!
//! No `Run` hosts a RocksDB directory: every driver passes
//! `rocksdb_descriptor: None`, so `runtime-next` makes and owns each shard's
//! own tempdir.
use anyhow::Context;
...
///
/// Dropping a `Run` is safe at any point, and [`Drop`] aborts the server and
/// admin tasks so they don't outlive the run.
pub struct Run {
    ...
    // Closed on `Run::drop`, which resolves the admin surface's
...
impl Drop for Run {
    fn drop(&mut self) {
        ...
        // otherwise the tonic server outlives this `Run`.
...
impl Run {
    /// Resources for driving a capture: only the optional admin surface.
    ...
    pub async fn start_capture(
        ...
        tracing::info!(
            ...
            "local capture services started",
        );
    ...
    /// Resources for driving a materialization or derivation: an ephemeral
    /// tonic server hosting `runtime_next::Service`, plus the shuffle log
    /// tempdir.
    ///
    /// The caller supplies the source of documents through `build_shuffle`,
    /// which receives the freshly-bound `peer_endpoint` — a live journal-reading
    /// run needs it to construct its loopback `shuffle::Service` — and returns
    /// the [`ShuffleSessionFactory`] plus an optional `shuffle::Service` to
    /// co-host. A run fed from pre-written segments returns `None` for the
        ...
        F: FnOnce(&str) -> anyhow::Result<(S, Option<shuffle::Service>)>,
    {
        ...
        // Only a live run serves peer Slice/Log RPCs; a segment-fed run reads
        ...
        tracing::info!(
            ...
            n_shards,
            debug_port = ?debug_port,
            "local leader + shuffle services started",
        );
...
fn spawn_admin_task(
    ...
    Some(tokio::spawn(async move {
        ...
        if let Err(err) = service_kit::admin::serve("runtime-local", registry, addr, shutdown).await
        {
            tracing::error!(?err, "runtime-local admin surface exited with error");
        }
    }))
}
