use super::{Annotation, FrozenString, Keyword, Schema};
    ...
    A: Annotation,
{
    ...
    pub fn into_index(self) -> Index<'s, A> {
        ...
        // aren't dynamic. Re-allocate URIs into FrozenString, making them likely
        // to be co-located in index order within the heap.
        let pruned: Vec<(FrozenString, bool, &'s Schema<A>)> = self
            .idx
            .into_iter()
