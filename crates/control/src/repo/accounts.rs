...
pub async fn fetch_all(db: &PgPool) -> Result<Vec<Account>, sqlx::Error> {
    sqlx::query_as!(
        Account,
        r#"
    SELECT id as "id!: Id", display_name, email, name, norm_name, created_at, updated_at
    FROM accounts
    "#
    )
    .fetch_all(db)
...
pub async fn insert(db: &PgPool, input: NewAccount) -> Result<Account, sqlx::Error> {
    ...
    let norm_name = input.name.clone();

    sqlx::query!(
        r#"
    INSERT INTO accounts(display_name, email, name, norm_name, created_at, updated_at)
