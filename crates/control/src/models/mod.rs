...
// Developer Note: The rest of the implementation for `Id<T>` looks complicated
// due to not being able to derive these traits automatically. The inner
// `PhantomData<T>` tends to break this for us. The implementations of these
// traits generally just defers to working with `value` and ignoring the `_type`
// outside of the type of Self.
...
// Manually implement some basic traits on `Id<T>`. Due to a limitation with the
// way fundamental trait derivation works, we cannot automatically derive them.
// If we do, it imposes restrictions on `T` that we do not want. eg. We want the
// `Id<Account>` to be `Copy`, even if `Account: !Copy`.
