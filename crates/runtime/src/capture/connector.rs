...
pub async fn start<L: LogHandler>(
    ...
    let mut connector_rx = match endpoint {
        ...
        models::CaptureEndpoint::Local(_) if !matches!(runtime.plane, crate::Plane::Local) => {
...
fn extract_endpoint<'r>(
    ...
    let verify = verify("client", "valid first request");
...
mod test {
    ...
    async fn test_http_ingest_spec() {
        ...
        assert!(matches!(resp.kind, Some(response::Kind::Spec(_))));
