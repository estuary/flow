use protocol::consumer::{self, shard_client::ShardClient};
use protocol::flow::{self, ingester_client::IngesterClient, testing_client::TestingClient};
...
impl Cluster {
    ...
    pub async fn list_journals(
        &self,
        selector: Option<broker::LabelSelector>,
    ) -> Result<broker::ListResponse, Error> {
        let mut client = JournalClient::connect(self.broker_address.clone()).await?;

        let request = tonic::Request::new(broker::ListRequest { selector });
    ...
    ) -> Result<tonic::Streaming<broker::ReadResponse>, Error> {
        let mut client = JournalClient::connect(self.broker_address.clone()).await?;
    ...
    ) -> Result<broker::ApplyResponse, Error> {
        let mut client = JournalClient::connect(self.broker_address.clone()).await?;
