...
#
#   go install go.gazette.dev/core/cmd/gazette    # from this checkout, whose go.mod pins it
...
DEMO_ADDRESS_DEFAULT="http://127.0.0.1:28080"
...
start() {
    ...
    # it ignores whatever token it is given. The key below is therefore a
    # placeholder, and a real data plane hands the daemon the same keys its brokers
    # verify with.
    nohup sudo "${DAEMON}" \
        --uds-path "${UDS}" \
        --broker-address "http://127.0.0.1:${BROKER_PORT}" \
        --data-plane-fqdn disk-daemon-demo \
        --data-plane-auth-keys ZGVtby1kaXNrLWRhZW1vbi1obWFjLXBsYWNlaG9sZGVy \
...
UDS_PATH=${UDS}
BROKER_ADDRESS=http://127.0.0.1:${BROKER_PORT}
EOF
    ...
    for expected in "UDS_PATH:${UDS}" "BROKER_ADDRESS:http://127.0.0.1:${BROKER_PORT}"; do
    ...
    if [[ http://127.0.0.1:${BROKER_PORT} != "${DEMO_ADDRESS_DEFAULT}" ]]; then
        echo "    export BROKER_ADDRESS=http://127.0.0.1:${BROKER_PORT}"
    fi
    echo "    cargo run -p disk-daemon --example basic"
    echo "    cargo run -p disk-daemon --example two_phase_commit"
    echo "    cargo run -p disk-daemon --example hot_standby"
    echo "    cargo run -p disk-daemon --example container"
    echo
...
stop() {
    ...
    sudo pkill -TERM -f "flow-disk-daemon --uds-path ${UDS}" 2>/dev/null || true
    await_gone "flow-disk-daemon --uds-path ${UDS}" sudo
...
status() {
    ...
    for named in "etcd:etcd --data-dir ${STATE}/etcd" \
        ...
        "the disk daemon:flow-disk-daemon --uds-path ${UDS}"; do
        local name=${named%%:*} pattern=${named#*:} pids

        pids=$(pgrep -d' ' -f "${pattern}" || true)
        echo "${name}: ${pids:-NOT running}"
    done
    echo "UDS_PATH=${UDS_PATH} BROKER_ADDRESS=${BROKER_ADDRESS}"
}
