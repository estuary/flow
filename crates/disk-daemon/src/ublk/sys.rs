//! ABI of `include/uapi/linux/ublk_cmd.h`, and the helpers this crate layers on
//! it.
//!
//! `build.rs` generates the definitions from the vendored header, so nothing here
//! is transcribed by hand. The opcodes are the ioctl-encoded `UBLK_U_*` forms. The
//! legacy raw opcodes need `CONFIG_BLKDEV_UBLK_LEGACY_OPCODES`, which current
//! kernels do not enable.
//!
//! This module renames the generated items to Rust casing. It also widens a few
//! constants to the types their fields use.

/// Generated from the vendored header, with compile-time assertions on the size,
/// alignment, and field offsets of every type. Every field is reached by name, so a
/// newer header cannot silently move data. A field this crate uses either still
/// exists or fails to compile.
mod bindings {
...
pub use bindings::{
    ...
    UBLK_IOCTL_GET_DEV_INFO as UBLK_U_CMD_GET_DEV_INFO,
    UBLK_IOCTL_GET_FEATURES as UBLK_U_CMD_GET_FEATURES,
...
/// Asks the kernel not to copy request data through a mapped per-queue buffer.
/// Data then moves by `pread` and `pwrite` against `/dev/ublkcN` instead.
pub const UBLK_F_USER_COPY: u64 = bindings::UBLK_F_USER_COPY as u64;

/// The kernel sets this on every device it creates. This crate reads it back to
...
/// Operation of a pending request. Its low eight bits carry that operation.
...
///
/// The header overlays this field with the zone count of a zoned device. This crate
/// does not serve a zoned device.
pub fn io_desc_sectors(desc: &UblksrvIoDesc) -> u32 {
    // SAFETY: both variants of the union are a `__u32`, so either one reads the
...
///
/// `UBLK_F_USER_COPY` needs no buffer address, and the kernel rejects a command
/// which supplies one. The union the header overlays there is therefore left zero.
...
/// logical block size. The kernel keeps this as `SECTOR_SHIFT` in a header which is
/// not uapi, so there is nothing to import.
...
/// [`io_desc_map_len`]. The kernel gives every queue room for the largest depth it
/// allows, whatever depth the device was created with.
...
pub fn page_size() -> usize {
    // SAFETY: sysconf reads no user memory, and it always answers for
    // _SC_PAGESIZE.
...
/// # Safety
///
/// `T` must be a `repr(C)` type with no interior padding. A padding byte would be
/// read uninitialized.
pub unsafe fn cmd_bytes<T: Copy, const N: usize>(value: &T) -> [u8; N] {
    assert!(std::mem::size_of::<T>() <= N);

    let mut bytes = [0u8; N];
    // SAFETY: the assertion above bounds the copy. `T: Copy` rules out any
