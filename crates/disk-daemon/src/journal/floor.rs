//! The recovery floor, which is where a replay of a disk journal may begin.
//!
//! The daemon derives the floor from journal contents, so it also persists it. It
//! writes it as a label on the journal's own spec, under a name each deployment
//! states. The floor is a seek position and never a filter. A filter on clock could
//! drop a record from the middle of a delta. A floor which is absent or behind
//! costs replay work and changes nothing about the result. A floor ahead of the
//! derived one is the one value which must never be written, because it would
//! invite the deletion of fragments a live disk still needs.
//!
//! The value encoding is fixed rather than configurable. It is sixteen lowercase
//! hex characters of the u64 message clock. Fixed-width hex orders
//! lexicographically as it does numerically. It also matches the
//! `estuary.dev/truncated-at` value Flow already writes, so tooling which reads
//! that label name generically cannot misparse a disk journal.
...
///
/// A fragment is persisted no earlier than the records it holds. A fragment
/// modified before the floor's clock therefore holds only records below the floor,
/// and the broker may skip it. Zero means the label is unnamed or absent, and the
/// replay begins at the first fragment available.
pub async fn seek(
...
///
/// The write is off the commit path, and best effort. A floor which does not reach
/// the label costs a later recovery some replay work and nothing else. No disk is
/// ever held up by one.
pub fn advance(
    client: gazette::journal::Client,
    journal: String,
...
///
/// The write is a compare-and-swap against the spec's revision, because the daemon
/// is not its only writer. A lost race means another writer changed the spec. The
/// watch reports that change as its next snapshot, and this then applies over it.
async fn write(
    client: &gazette::journal::Client,
    journal: &str,
    label: &str,
...
/// The change which sets `label` to `value` on the listed `journal`. `None` if the
/// journal already holds that value or a later one, and `None` for a journal which
/// has since been deleted.
///
/// This mutates the listed spec and applies it back, rather than rebuilding one
/// from the daemon's own inputs. The listed spec carries fields the daemon does not
/// model, notably the `suspend` Gazette sets when it idles a journal.
fn change(
    ...
    // This one comparison therefore covers an equal floor and a later one.
    if labels
        .iter()
...
mod test {
        ...
        broker::ListResponse {
            journals: vec![broker::list_response::Journal {
                spec: Some(broker::JournalSpec {
                    ...
                    // A field the daemon does not model. The spec it applies back
                    // must still carry it.
    ...
    fn test_the_floor_label_only_advances() {
        ...
        for held in [Vec::new(), vec![(LABEL, "0000000000000010")]] {
            ...
            // The listed spec is applied back, so what the daemon does not model
            // survives the write.
    ...
    /// The label joins the spec's own labels. Those stay sorted as Gazette
    ...
    /// A journal which is not in the listing has been deleted. There is nothing
    /// left to label.
    #[test]
