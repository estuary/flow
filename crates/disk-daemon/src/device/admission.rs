...
//! admission — parks its request in arrival order. Every later mutation parks behind
//! it, and `retry_parked` offers them again without reordering them.
//!
//! Compaction is here for the same reason: a horizon copy is a mutation the owner
//! offers itself, out of the budget the delta's own traffic earned. The disk's own
//! requests come first, so no copy is offered while one of them is parked.
...
impl Owner {
    ...
    /// places it after the cut. So does a mutation already parked, which a later one
    /// must not overtake into room the channel has just freed.
    ///
    ...
    pub(super) fn offer(
        ...
        let offered = match self.admitting && self.parked.is_empty() {
    ...
    /// closed admission. This keeps arrival order, and nothing is offered ahead of
    /// a parked request, so neither backpressure nor a cut reorders two mutations.
