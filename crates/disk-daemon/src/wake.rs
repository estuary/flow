...
/// an ordinary completion. An eventfd counts rather than latches, so a wake
/// which lands between completions is not lost: the next armed read returns
/// immediately.
#[derive(Clone)]
...
impl Waker {
    pub fn new() -> std::io::Result<Self> {
        ...
        // SAFETY: the descriptor is freshly created and owned by nothing else,
        // so `File` may take responsibility for closing it.
    ...
    pub fn wake(&self) {
        // An eventfd is not seekable, so this is a plain write. A counter
        // overflow is the only way it fails, and the owner drains the counter on
        // every read.
