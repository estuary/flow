# materialize-consistency: operating guide

The complete operating reference for this suite, written to be sufficient for an
agent (or a person) driving it unattended: every requirement, every gotcha, and
what each kind of failure means. For a quick human orientation, read `README.md`
first.

The suite runs a connector as a real task on a real Flow runtime, deliberately
breaks it at precise points, and checks that the destination still holds exactly
the right data.
