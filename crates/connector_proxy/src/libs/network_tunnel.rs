...
impl NetworkTunnel {
    pub fn extend_endpoint_schema(
        ...
        let network_tunnel_schema = &mut create_root_schema::<NetworkTunnelConfig>();
        ...
        if let Some(ref mut o) = &mut modified_schema.schema.object {
            ...
            if o.as_ref().properties.contains_key(ENDPOINT_ADDRESS_KEY) {
                visit_root_schema(&mut RemoveSSHForwardHost, network_tunnel_schema);
            }

            o.as_mut().properties.insert(
                NETWORK_TUNNEL_KEY.to_string(),
                Schema::Object(network_tunnel_schema.schema.clone()),
            );
        }
