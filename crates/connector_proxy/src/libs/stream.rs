...
use crate::errors::{Error, interceptor_stream_to_io_stream, io_stream_to_interceptor_stream};
...
pub fn stream_lines(
    ...
    io_stream_to_interceptor_stream(
        AsyncByteLines::new(StreamReader::new(interceptor_stream_to_io_stream(in_stream)))
        .into_stream()
        .map_ok(Bytes::from)
    )
}
...
pub fn stream_runtime_messages<T: prost::Message + std::default::Default>(
    ...
    let reader = StreamReader::new(in_stream.map_err(|e| create_custom_error(&e.to_string())));
