...
pub struct FireAlert {
    ...
    pub notification_task_id: models::Id,
...
pub struct ResolveAlert {
    pub notification_task_id: Option<models::Id>,
    pub catalog_name: String,
    pub alert_type: AlertType,
...
pub struct Alert {
    /// The id of the notification task for this alert. This should always be
    /// `Some` for any alerts that are open, or that have not yet had all
    /// notifications sent. Once all notifications have been sent for a given
    /// alert, this will be `None`.
    #[graphql(skip)]
    pub notification_task_id: Option<models::Id>,
...
pub async fn fetch_open_alerts_by_catalog_name(
    ...
    let result = rows
        .into_iter()
        .map(|row| Alert {
            notification_task_id: row.notification_task_id,
...
pub async fn fetch_alert_by_task_id(
    notification_task_id: models::Id,
    db: &sqlx::PgPool,
    ...
    Ok(Some(Alert {
        notification_task_id: alert.notification_task_id,
...
async fn fire_alert(
    ...
    tracing::info!(%notification_task_id, %catalog_name, %alert_type, %fired_at, "fired alert");

    Ok(())
}

async fn resolve_alert(
    notification_task_id: Option<models::Id>,
    catalog_name: &str,
    alert_type: AlertType,
    ...
    let notification_task_id = notification_task_id.unwrap_or(models::Id::zero());
    tracing::info!(%notification_task_id, %catalog_name, %alert_type, %resolved_at, "resolved alert");

    Ok(())
}

pub async fn remove_notification_task_id(
    task_id: models::Id,
    txn: &mut sqlx::PgConnection,
) -> anyhow::Result<()> {
    sqlx::query!(
        r#"update alert_history
        set notification_task_id = null
        where notification_task_id = $1"#,
