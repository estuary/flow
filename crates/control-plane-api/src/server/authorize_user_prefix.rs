...
fn evaluate_authorization(
    ...
    if !authority.is_authorized(prefix, capability) {
    ...
    if !authority.is_authorized(data_plane_name, models::Capability::Read) {
        return Err(tonic::Status::permission_denied(format!(
...
mod tests {
    ...
    async fn test_user_prefix_auth_success(pool: sqlx::PgPool) {
        ...
        let source = flow_client::workflows::UserPrefixAuth {
    ...
    async fn test_user_prefix_auth_failure(pool: sqlx::PgPool) {
        let _guard = test_server::init();
        ...
        Status {
            ...
            message: "alice@example.com is not authorized to Some/Other/Prefix/ for Write",
            source: None,
        }
        "#,
        );
