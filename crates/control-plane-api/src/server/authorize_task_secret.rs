...
///
///  * Its `sub` is the shard subject requesting decryption, or for Discover and
///    Validate RPCs proxied from the control plane, a synthetic shard ID having
///    an all-zero generation ID and full key / r-clock range. Note we must handle
///    requests for tasks that have not yet been published, and cannot require that
///    task shards are in the Snapshot as `/authorize/task` does.
///
...
/// The task name of a *synthetic* Shard ID: the all-zero generation ID over a
/// range which fully covers the key and r-clock spaces, which Discover and
/// Validate use to name a task that has no shard. It reads as what it is: not a
/// split of some generation, but the task itself.
///
/// A Shard ID of a real shard is resolved through the Snapshot instead, so the
/// generation and range are matched exactly rather than merely shape-checked.
/// Task names have arbitrarily many components, and an exact suffix is what
/// makes it impossible for a truncated Shard ID to yield a shorter -- and thus
/// more privileged -- name than the caller meant.
fn synthetic_task_name(shard_id: &str) -> Option<&str> {
    let prefix = shard_id.strip_suffix("/0000000000000000/00000000-00000000")?;
