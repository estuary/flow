...
pub async fn handle_post_token(
    ...
    match req {
        ...
        // (id, secret) pair in the body, and `env`'s claims go unused. A
        // request which nonetheless presents an Authorization bearer has it
        // verified by extraction above, so a broken bearer is rejected
        // regardless of grant.
...
async fn mint_capability_token(
    ...
    if crate::grants::is_service_account(&env.pg_pool, claims.sub).await? {
...
/// shape is pinned by the pgTAP tests under `supabase/tests/`; the two mint
...
/// `models::authz::CapabilityMask::from_claim`.
...
#[cfg(test)]
mod test {
    use crate::test_server::{self, ALICE};
    ...
    fn test_validity_is_configurable() {
        let caller = test_server::control_claims(ALICE, None, None);
    ...
    /// enforcement at the status route; every refusal (missing bearer,
    ...
    async fn test_capability_token_mint(pool: sqlx::PgPool) {
        ...
        // and still be functionally identity-only. Driving the status route
        // (which requires Read) with minted tokens closes that seam through
        // real extraction: the Viewer-bearing mask covers the requirement and
        // reaches the walk (the 307 is the walk's provisional denial under
        // the test's empty first Snapshot), while the identity-only mask is
        // the definitive structured 403, naming the shortfall.
        ...
        assert_eq!(
            ...
            "a minted Viewer mask covers the requirement and reaches the walk: {body}"
        );
        ...
        assert!(
            body["missing_capabilities"]
                .as_array()
                .unwrap()
                .contains(&serde_json::json!("CatalogRead")),
            "the shortfall names the Read capabilities: {body}"
        );
        ...
        // credential) distinguishes copy-through, since role fidelity is what
        ...
        let scoped_role_token = {
            let mut claims = test_server::control_claims(ALICE, None, None);
            claims.role = "github_action_connector_refresh".to_string();
            jsonwebtoken::encode(
        ...
        let masked_svc_token = server.make_masked_access_token(svc_user, None, &["CatalogRead"]);
        let (status, body) = post_token(
            &server,
            &capability_request(serde_json::json!(["CatalogRead"])),
