...
//! per-surface enforcement with hand-signed masked tokens lives in each
//! surface's own module, and several of the composed properties are already
//! pinned there with minted tokens. A minted token cannot re-mint and the
//! mint refuses service accounts (`public/token_exchange`); a masked bearer
//! cannot `createRefreshToken` (`public/graphql/refresh_tokens`); and the
//! `/admin` routes deny a masked bearer through the same masked walk as
//! every other surface, since they authorize `ops/` Admin via
//! `Envelope::verify_authorization`.
//!
...
/// on refusal. Walk-refusal bodies carry the caller's email, so outcome
/// strings double as the identity-copy-through probe for minted bearers,
/// while mask-shortfall refusals are the structured `Forbidden` body.
