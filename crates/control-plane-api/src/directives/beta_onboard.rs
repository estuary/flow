...
/// `DataMovementStalled` threshold seeded into a new tenant's `alert_configs`
/// row. Twelve hours is an onboarding signal — "your task never started moving
/// data" — rather than a tight freshness SLA, and the controller's age gate
/// gives every new task at least this long before it can fire.
...
pub async fn provision_tenant(
    ...
    // Seeded at the tenant prefix — the broadest layer of the alert_configs
    // hierarchy — so more-specific prefix or per-task rows override it, and
    // tenant-wide tuning is an edit of this row rather than a new mechanism.
