...
pub struct UncommittedBuild {
    ...
    pub build_id: models::Id,
    pub user_id: Uuid,
    pub detail: Option<String>,
    pub started_at: DateTime<Utc>,
    pub output: build::Output,
    pub test_errors: tables::Errors,
    pub retry_count: u32,
}
...
impl Publisher {
    ...
    /// Skip running catalog tests as part of publications. This is used in
    ...
    /// Temporarily exported function for publications executor
    ...
    pub async fn build(
            ...
            && built.errors().next().is_none()
        {
            ...
            let errors = builds::test_catalog(logs_token, &self.logs_tx, &built).await?;
    ...
    pub async fn commit<C: WithCommit>(
        &self,
