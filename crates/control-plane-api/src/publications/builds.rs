...
/// steps run against an in-memory collection store. No broker or etcd is
/// involved, so there is nothing to activate beforehand or delete afterward.
///
/// Connector and runtime logs stream to the publication's job logs, gated by
/// each task's own `shards: {logLevel}` — so a user who wants a derivation's
/// debug output in their publication logs raises that task's level.
pub async fn test_catalog(
    ...
    let options = catalog_tests::Options {
        ...
        splits: 3, // Exercise multi-shard key routing.
