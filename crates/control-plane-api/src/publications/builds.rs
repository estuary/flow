...
/// Run a built catalog's tests locally, returning a `tables::Error` per failing
/// test case.
///
/// Derivations execute as resident runtime-next sessions — derive-sqlite
/// in-process, image derivations as containers on `connector_network`, with three
/// splits to exercise multi-shard key routing — and each test's ingest / verify
/// steps run against an in-memory collection store. No Gazette broker, etcd, Go
/// consumer, or `flowctl-go` binary is involved, so there is nothing to activate
/// beforehand or delete afterward. Connector and runtime logs stream to the
...
pub async fn test_catalog(
    ...
    // Stream ops logs to the publication's job logs under the same "test" stream
    // name the `flowctl-go api test` job used, so existing log consumers are
    // unaffected.
