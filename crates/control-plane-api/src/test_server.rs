...
/// observes a periodic snapshot refresh, with no restart and no token
...
pub fn verified_control_claims(
    ...
    let claims = control_claims(user_id, email.as_deref(), None);
...
pub async fn envelope(
    ...
    crate::Envelope {
        ...
        capability_mask: crate::CapabilityMask::ALL_CAPABILITIES,
    }
}
