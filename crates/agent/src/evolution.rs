...
impl Handler for EvolutionHandler {
    async fn handle(&mut self, pg_pool: &sqlx::PgPool) -> anyhow::Result<HandlerStatus> {
        ...
        let id: Id = row.id;
        let status = match process_row(row, &mut txn).await {
            Ok(updated) => {
                serde_json::json!({
                    "type": "success",
                    "evolved_collections": updated,
                })
            }
            Err(err) => {
                serde_json::json!({
...
async fn process_row(
...
) -> anyhow::Result<Vec<EvolvedCollection>> {
    let Row {
        draft_id,
        user_id,
        collections,
    ...
    validate_evolving_collections(&draft_specs, &collections)?;
    ...
    let update_helper = ResourceSpecUpdater::for_catalog(txn, &before_catalog)
        .await
        .context("processing connector resource spec schemas")?;
...
fn validate_evolving_collections(
        ...
        .filter(|r| evolving_collections.contains_key(r.catalog_name.as_str()))
    {
        ...
        if !collection_name_regex.is_match(new_name.as_str()) {
            anyhow::bail!("requested collection name '{new_name}' is invalid");
        }

        // Validate that the user has the admin capability for any collection
        // they're requesting to re-create. We do the permission validations
        // first so that we don't accidentally leak information about
        // unauthorized entities in the error messages. Note that technically,
        // you don't need `admin` on the _old_ collection name, as long as
        // you have it on the new name. But it seems like a good precedent to
        // require `admin`, since in the future we may wish to somehow mark the
        // old collection as deprecated.
        if row.user_capability != Some(Capability::Admin) {
            anyhow::bail!("you are not authorized to 'admin' the spec '{old_name}'");
        }
        ...
        // Technically, these checks duplicate validations that would already be performed if the
        // user tries to publish the draft after re-creating the collections. But this allows us
        // to surface the error more quickly.
        match row.live_type {
            None => {
                anyhow::bail!("cannot re-create collection '{old_name}' which was already deleted")
            }
            Some(CatalogType::Collection) => { /* all good */ }
            Some(other) => {
                anyhow::bail!("the entity '{old_name}' is a {other}, expected a collection")
            }
        }
    }
