...
fn update_live_specs(
    ...
    for row in captures.into_iter() {
        ...
        let spec = spec.expect("deletions are not yet supported in tests");
        let new_live = tables::LiveCapture {
            live_spec_id: id_gen.next_id(),
            last_build_id: pub_id,
            catalog_name,
            spec,
            built_spec: Default::default(),
        };
        live.captures.upsert(new_live, |new_row, prev_row| {
            if let Some(prev) = prev_row {
                new_row.live_spec_id = prev.live_spec_id;
            } else {
                new_row.live_spec_id = id_gen.next_id();
            }
        });
    }
    for row in collections.into_iter() {
        ...
        live.collections.upsert(new_live, |new_row, prev_row| {
            if let Some(prev) = prev_row {
                new_row.live_spec_id = prev.live_spec_id;
            } else {
                new_row.live_spec_id = id_gen.next_id();
    ...
    for row in materializations.into_iter() {
        ...
        live.materializations.upsert(new_live, |new_row, prev_row| {
            if let Some(prev) = prev_row {
                new_row.live_spec_id = prev.live_spec_id;
            } else {
                new_row.live_spec_id = id_gen.next_id();
    ...
    for row in tests.into_iter() {
        ...
        live.tests.upsert(new_live, |new_row, prev_row| {
            if let Some(prev) = prev_row {
                new_row.live_spec_id = prev.live_spec_id;
            } else {
                new_row.live_spec_id = id_gen.next_id();
    ...
    let _ = live
        ...
        .outer_join_mut(
            built_captures
                .into_iter()
                .map(|r| (r.catalog_name.clone(), r)),
            |eob| match eob {
                EitherOrBoth::Both(live, (_, built)) => {
                    live.built_spec = built.spec;
                    Some(1)
                }
                _ => None,
            },
    ...
    let _ = live
        .tests
        .outer_join_mut(
            built_tests.into_iter().map(|r| (r.catalog_name.clone(), r)),
            |eob| match eob {
                EitherOrBoth::Both(live, (_, built)) => {
                    live.built_spec = built.spec;
                    Some(1)
...
impl ControlPlane for MockControlPlane {
    ...
    async fn get_inferred_schemas(
        ...
        let schemas = self
            .live
            .inferred_schemas
            .inner_join(collections.iter().map(|c| (c, ())), |s, _, _| {
                Some(s.clone())
            })
            .collect();
