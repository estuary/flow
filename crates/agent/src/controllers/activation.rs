...
use models::{
    status::activation::{ActivationStatus, ShardFailure},
    Id,
};
...
pub async fn update_activation<C: ControlPlane>(
    ...
    if got_shard_failed || status.recent_failure_count > 0 {
        ...
        if got_shard_failed && status.next_retry.is_none() {
    ...
    if let Some(rt) = status.next_retry {
        if rt <= now {
            tracing::info!("restarting failed task shards");
            do_activate(now, state, status, control_plane).await?;
        } else {
...
fn get_next_retry_time(now: DateTime<Utc>, failures: &[ShardFailure]) -> Option<DateTime<Utc>> {
    ...
    // TODO: delete this
    eprintln!("next_retry: {next:?}, consecutive_failures: {consecutive_failures}, shard_count: {shard_count}, failures: {}", failures.len());
    tracing::info!(
        ?next,
        %now,
