...
impl automations::Executor for TagExecutor {
    ...
    async fn poll<'s>(
        ...
        tracing::info!(%time_queued, id = %task_id, status = ?next_status, "finished");
        inbox.clear();
        Ok(TagOutcome {
            id: task_id,
            status: next_status,
    ...
    async fn process(&self, row: Row, pool: &sqlx::PgPool) -> anyhow::Result<JobStatus> {
        ...
        let data_plane_id = loop {
            ...
            if let Some(data_plane_id) = snapshot
                .data_planes()
                .find(|plane| {
                    plane.data_plane_name.starts_with("ops/dp/public/")
                        && !plane.closed
                        && plane.can_sign()
                })
        ...
        // Determine the prior protocol of the connector tag, if one exists.
        let protocols: Vec<String> = sqlx::query_scalar!(
            r#"SELECT DISTINCT protocol AS "protocol!" FROM connector_tags
               WHERE connector_id = $1 AND protocol IS NOT NULL ORDER BY protocol"#,
            row.connector_id as models::Id,
        )
        .fetch_all(pool)
        .await?;
        ...
        let spec_result = match protocol {
            ...
            // Probe both protocols, taking the first that succeeds.
