...
/// specs in the build. Note that this must be used in conjuction with the `ExpandDraft`
/// `Initialize` impl, or else it can prune collections that are referenced by live specs
/// that are not included in the draft.
