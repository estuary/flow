...
async fn test_alert_notifications() {
    ...
    sqlx::raw_sql(r#"
        insert into alert_history
          (notification_task_id, catalog_name, alert_type, fired_at, resolved_at, arguments, resolved_arguments)
          values
            ...
            select notification_task_id, 9 from alert_history;
    ...
    let resolve = ResolveAlert {
        notification_task_id: Some(happy_id),
    ...
    assert_eq!(failures_id, failed_run_1.notification_task_id);
    ...
    resolve_alert(
        ResolveAlert {
            notification_task_id: Some(failures_id),
    ...
    for (task_id, catalog_name) in no_subscriber_ids {
        ...
        resolve_alert(
            ResolveAlert {
                notification_task_id: Some(task_id),
...
async fn test_data_movement_stalled() {
    ...
    sqlx::raw_sql(r#"
        ...
        insert into catalog_stats_hourly (catalog_name, grain, ts, flow_document, bytes_written_by_me, bytes_written_to_me, bytes_read_by_me) values
          ...
          -- Must disable triggers so that we don't attempt to send email notifications using the legacy trigger.
          alter table alert_history disable trigger user;

          -- Simulate an alert that was fired by the previous version of the system, and does not have
          -- a notification_task_id. Expect that this alert will get resolved by the new system.
          insert into alert_history (notification_task_id, catalog_name, alert_type, fired_at, arguments)
          values
            (null, 'aliceCo/materialization/legacy', 'data_movement_stalled'::alert_type, '2025-01-01T01:02:03Z', '{
                "bytes_processed": 0,
            ...
            (null, 'aliceCo/capture/deleted', 'data_movement_stalled'::alert_type, '2025-01-01T01:02:03Z', '{
                "bytes_processed": 0,
                "evaluation_interval": "02:00:00",
                "recipients": [
                {
          ...
          alter table alert_history enable trigger user;
        "#)
    ...
    let fired = harness
        ...
        .await;
    let fired_notification_task_id = fired.notification_task_id;
    ...
    assert!(
        legacy_alert.notification_task_id.is_none(),
        "expect notification_task_id to still be unset for legacy alert"
    );
    assert!(legacy_alert.resolved_at.is_none());

    // Assert that the legacy alert for the deleted spec got resolved
    let legacy_deleted_alert = sqlx::query!(
        r#"select
            notification_task_id as "notification_task_id: models::Id",
            resolved_at,
            resolved_arguments as "resolved_arguments: sqlx::types::Json<serde_json::Value>"
        from alert_history
        where catalog_name = 'aliceCo/capture/deleted'
        and alert_type = 'data_movement_stalled'::alert_type
        "#
    )
    .fetch_one(&harness.pool)
    .await
    .expect("fetching legacy alert");

    assert!(
        legacy_deleted_alert.notification_task_id.is_none(),
        "expected no notification_task_id, got: {legacy_alert:?}"
    );
    assert!(
        legacy_deleted_alert.resolved_at.is_some(),
        "expected resolved_at to be some, got: {legacy_alert:?}"
    );
    assert!(
        legacy_deleted_alert.resolved_arguments.is_none(),
        "expected resolved_arguments to be none, got: {legacy_alert:?}"
    );
