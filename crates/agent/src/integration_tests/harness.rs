...
impl TestHarness {
    ...
    pub async fn assert_alert_firing(&mut self, catalog_name: &str, alert: AlertType) -> TestAlert {
        ...
        let notification_task_id = open_alert
            .notification_task_id
            .expect("alert must have a notification_task_id");
    ...
    /// Asssert that the alert for the given `notification_task_id` gets resolved
    pub async fn assert_alert_resolved(&mut self, notification_task_id: models::Id) -> TestAlert {
        let Some(alert) =
            control_plane_api::alerts::fetch_alert_by_task_id(notification_task_id, &self.pool)
                .await
                .unwrap()
        else {
