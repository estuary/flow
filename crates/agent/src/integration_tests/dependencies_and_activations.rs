...
async fn test_shard_failures_and_retries() {
    ...
    for expect_count in 1..6 {
        ...
        assert!(shard_health.ts > last_check);
        last_check = shard_health.ts;
...
async fn override_shard_status_check_at(
    ...
    tracing::debug!(%catalog_name, %new_ts, "overriding activation shard_health ts");
    sqlx::query!(
        r#"update controller_jobs set
        status = jsonb_set(status::jsonb, '{activation, shard_health, ts}', to_jsonb($2::text))::json
        where live_spec_id = (select id from live_specs where catalog_name = $1)
        and status->'activation'->'shard_health'->>'ts' is not null
        returning 1 as "must_exist: bool";"#,
        catalog_name,
        new_ts,
    )
