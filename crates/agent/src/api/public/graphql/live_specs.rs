...
pub async fn fetch_live_specs(
    ...
    let rows = sqlx::query!(
        r#"select
                ...
                case when $3 then ls.spec::text else null end as "spec: agent_sql::TextJson<async_graphql::Value>",
                ls.last_build_id as "last_build_id: models::Id",
                ls.last_pub_id as "last_pub_id: models::Id",
                ls.created_at,
                ls.updated_at,
                case when $4 then ls.built_spec::text else null end as "built_spec: agent_sql::TextJson<async_graphql::Value>"
