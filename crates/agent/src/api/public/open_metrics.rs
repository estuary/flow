...
use crate::api::{public::ApiErrorExt, snapshot::Snapshot, App, ControlClaims};
...
pub async fn handle_get_metrics(
    ...
    axum::Extension(ControlClaims {
        sub: user_id, iat, ..
    }): axum::Extension<ControlClaims>,
) -> axum::response::Response {
    let mut prefixes: Vec<String> = loop {
        match Snapshot::evaluate(&state.snapshot, iat, |snapshot| {
            Ok(tables::UserGrant::transitive_roles(
                &snapshot.role_grants,
                &snapshot.user_grants,
                user_id,
            )
            .filter(|grant| grant.capability >= models::Capability::Read)
            .map(|grant| grant.object_role.to_string())
            .collect())
        }) {
            Err(Ok(millis)) => tokio::time::sleep(std::time::Duration::from_millis(millis)).await,
            Err(Err(err)) => {
                return err
                    .with_status(axum::http::StatusCode::UNAUTHORIZED)
                    .into_response()
            }
            Ok(prefixes) => break prefixes,
        }
    };
    prefixes.sort();
    prefixes.dedup_by(|lhs, rhs| rhs.starts_with(&*lhs));
