...
async fn do_update_l2_reporting(
    ...
    let (Some(mut l2_stats), Some(mut l2_inferred), Some(mut l2_status)) =
    ...
    let models::Derivation {
        transforms: l2_status_transforms,
        ..
    } = &mut l2_status.model.as_mut().unwrap().derive.as_mut().unwrap();
...
export class Derivation extends Types.IDerivation {"#
    ...
    for data_plane in &data_planes {
        ...
        l2_status_transforms.push(models::TransformDef {
    ...
    *l2_stats_module_raw = models::RawValue::from_value(&serde_json::json!(l2_stats_module));
    ...
    let draft = tables::DraftCatalog {
        collections: tables::DraftCollections::from_iter([l2_inferred, l2_stats, l2_status]),
        ..Default::default()
    };
