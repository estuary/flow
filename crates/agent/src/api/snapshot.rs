    ...
    let migrations = sqlx::query_as!(
        ...
        r#"
            select
                ...
                sp.id as "src_plane_id: models::Id",
                tp.id as "tgt_plane_id: models::Id",
                m.cordon_at as "cordon_at!"
            from internal.migrations m
            join public.data_planes sp on m.src_plane_name = sp.data_plane_name
            join public.data_planes tp on m.tgt_plane_name = tp.data_plane_name
            where m.cordon_at is not null
            "#,
    )
    .fetch_all(pg_pool)
    .await
