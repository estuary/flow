...
async fn do_authorize(app: &App, token: &str) -> anyhow::Result<Response> {
    ...
    tracing::info!(?claims, ?header, "decoded token");
    ...
    claims.cap &= !proto_flow::capability::AUTHORIZE; // Strip bit.
    ...
    // Attempt to find an HMAC key of this data-plane which validates against token.
    ...
    // to its designated partition of its ops collections.
    if !authorized
        && required_role == "write"
    ...
    let key = jsonwebtoken::EncodingKey::from_base64_secret(
