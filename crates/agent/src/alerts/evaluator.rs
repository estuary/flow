...
pub fn evaluate_alert_actions(
    ...
    let mut indexed_open: HashMap<
        (String, AlertType),
        (Option<models::Id>, DateTime<Utc>, ArgsObject),
    > = open_alerts
        .into_iter()
        .map(|alert| {
            let Alert {
            ...
            let value = (notification_task_id, fired_at, arguments.0);
            (key, value)
        })
        .collect();
    ...
    for view_alert in current_alert_state {
        ...
        if let Some((_open_key, (notification_task_id, _fired_at, _open_args))) =
            indexed_open.remove_entry(&key)
        {
            if !firing {
            ...
            if firing {
                let (catalog_name, alert_type) = key;
                let notification_task_id = id_gen.next();
    ...
    for ((catalog_name, alert_type), (notification_task_id, _fired_at, _)) in indexed_open {
        actions.push(AlertAction::Resolve(ResolveAlert {
            notification_task_id,
            catalog_name,
            alert_type,
...
impl<V: AlertView> AlertEvaluator<V> {
    ...
    async fn try_evaluate(
        ...
        // TODO: don't clone an IdGenerator!!!!!
