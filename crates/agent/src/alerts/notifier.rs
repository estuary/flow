...
use control_plane_api::alerts::{Alert, fetch_alert_by_task_id};
...
impl<ES: EmailSender> AlertNotifications<ES> {
    ...
    async fn try_send_notifications(
        ...
        if is_resolved {
            Ok(AlertOutcome::ResolvedSent { task_id })
        } else {
...
pub enum AlertOutcome {
    AwaitResolution,
    ResolvedSent { task_id: models::Id },
    BackoffErr { backoff: std::time::Duration },
}

impl automations::Outcome for AlertOutcome {
    async fn apply<'s>(self, txn: &'s mut sqlx::PgConnection) -> anyhow::Result<Action> {
        match self {
            AlertOutcome::AwaitResolution => Ok(Action::Suspend),
            AlertOutcome::ResolvedSent { task_id } => {
                control_plane_api::alerts::remove_notification_task_id(task_id, txn)
                    .await
                    .context("removing notification_task_id")?;
                Ok(Action::Done)
            }
...
impl<ES: EmailSender> Executor for AlertNotifications<ES> {
    ...
    async fn poll<'s>(
        ...
        let Some(alert) = fetch_alert_by_task_id(task_id, pool)
            .await
            .context("fetching from alert_history")?
        else {
            tracing::warn!(%task_id, ?state, "no alert found for task");
            return Ok(AlertOutcome::ResolvedSent { task_id });
        };
        ...
        match self.try_send_notifications(state, task_id, alert).await {
            Ok(result) => {
...
fn into_alert_state(alert: Alert) -> notifications::AlertState {
    ...
    notifications::AlertState {
        alert_id: notification_task_id
            .expect("notification task id cannot be None if there's an AlertNotification task"),
