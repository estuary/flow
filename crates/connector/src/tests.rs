...
/// spec, has no identity to authorize and is `InvalidArgument` — as is one
/// which names `<spec>`. The sentinel is reserved because every minted bearer
/// admits it, and a session under it skips unsealing and IAM injection.
#[test]
fn task_identity_rejects_unauthorizable_requests() {
    ...
    for (request, message) in [
        ...
        (
            derive_open(crate::SPEC_TASK_NAME),
            "`<spec>` is reserved for Spec requests and cannot name a task",
        ),
    ] {
        let err = proto_grpc::connector::task_identity(&request).unwrap_err();
...
async fn sqlite_vfs_uri_is_rejected_for_other_connectors() {
    // Driven in-process, because a wire client may not set `sqlite_vfs_uri`
    // at all: see `a_wire_client_cannot_send_runtime_internal_input`.
    let (_service, router) = local_service();
...
/// The wire path refuses input which only an in-process caller may send,
/// before any connector is started: a runtime-internal `sqlite_vfs_uri` (an
/// unvalidated path which the reactor would open and create), and the `<spec>`
/// sentinel naming a task, which any bearer at all would authorize.
#[tokio::test]
async fn a_wire_client_cannot_send_runtime_internal_input() {
    let dir = tempfile::tempdir().unwrap();
    let vfs_uri = dir.path().join("probe.db");
    let config = serde_json::json!({"image": "busybox:latest", "config": {}});

    let mut rendered = Vec::new();

    for (task_type, task_name, request) in [
        (
            ops::TaskType::Derivation,
            "acmeCo/derivation",
            proto::Request {
                start: Some(start(&vfs_uri.to_string_lossy())),
                kind: Some(derive_open("acmeCo/derivation")),
            },
        ),
        (
            ...
            proto::Request {
                ...
                kind: Some(proto::request::Kind::Capture(capture::Request {
                    kind: Some(capture::request::Kind::Validate(Box::new(
                        capture::request::Validate {
                            name: crate::SPEC_TASK_NAME.to_string(),
        ...
        rendered.extend(render(
            drive_loopback(service, metadata, vec![request]).await,
        ));
    }
