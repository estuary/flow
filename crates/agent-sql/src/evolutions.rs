use crate::{publications::RoleGrant, Capability, CatalogType, Id, TextJson as Json};
use chrono::prelude::*;
use serde::Serialize;
    ...
    sqlx::query_as!(
        ...
        r#"
            select
                ...
                l.catalog_name,
                l.spec as "spec: Json<Box<RawValue>>",
                l.spec_type as "spec_type: CatalogType",
                coalesce(
                    (select json_agg(row_to_json(role_grants))
                    from role_grants
                    where starts_with(l.catalog_name, subject_role)),
                    '[]'
            ...
            from live_specs l
            where l.catalog_name = ANY($1)
        "#,
        catalog_names.as_slice(),
        user_id,
    )
    .fetch_all(txn)
    .await
