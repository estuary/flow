...
| The authenticated route works end-to-end | `#[sqlx::test]` through the real router: 401 without a token; with a minted Supabase-style JWT, 200 + a complete AG-UI SSE stream (snapshot-tested through the real router, CORS/trace layers included). |
