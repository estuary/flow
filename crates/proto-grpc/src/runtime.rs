...
pub mod task_control_client {
    ...
    use tonic::codegen::http::Uri;
    use tonic::codegen::*;
    /// TaskControl is a user-facing control surface for running tasks.
    /// It's distinct from the Leader service because it has a different
    /// authorization model: callers present ordinary gazette READ claims
    /// scoped to the task's shards, not the LEAD capability held by shards.
    ///
    /// The reactor front door serves TaskControl. Users reach it as HTTP/NDJSON
    /// (see go/runtime/task_control_http.go); gRPC carries only the front door's
    /// internal hop to the peer front door which is the task's shard-zero primary.
    #[derive(Debug, Clone)]
    pub struct TaskControlClient<T> {
        inner: tonic::client::Grpc<T>,
    }
    impl TaskControlClient<tonic::transport::Channel> {
        /// Attempt to create a new client by connecting to a given endpoint.
        pub async fn connect<D>(dst: D) -> Result<Self, tonic::transport::Error>
        where
            D: TryInto<tonic::transport::Endpoint>,
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        pub fn with_interceptor<F>(
            inner: T,
            interceptor: F,
        ) -> TaskControlClient<InterceptedService<T, F>>
        where
            F: tonic::service::Interceptor,
            T::ResponseBody: Default,
            T: tonic::codegen::Service<
            ...
            <T as tonic::codegen::Service<http::Request<tonic::body::Body>>>::Error:
                Into<StdError> + std::marker::Send + std::marker::Sync,
        {
            TaskControlClient::new(InterceptedService::new(inner, interceptor))
        }
        /// Compress requests with the given encoding.
        ///
        /// This requires the server to support it otherwise it might respond with an
        ...
        pub async fn sync_now(
            &mut self,
            request: impl tonic::IntoRequest<::proto_flow::runtime::SyncNowRequest>,
        ) -> std::result::Result<
            tonic::Response<tonic::codec::Streaming<::proto_flow::runtime::SyncNowResponse>>,
            tonic::Status,
        > {
            self.inner.ready().await.map_err(|e| {
                tonic::Status::unknown(format!("Service was not ready: {}", e.into()))
            ...
            let codec = tonic_prost::ProstCodec::default();
            let path = http::uri::PathAndQuery::from_static("/runtime.TaskControl/SyncNow");
            let mut req = request.into_request();
            req.extensions_mut()
                .insert(GrpcMethod::new("runtime.TaskControl", "SyncNow"));
            self.inner.server_streaming(req, path, codec).await
        }
    }
}
...
pub mod task_control_server {
    ...
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with TaskControlServer.
    #[async_trait]
    pub trait TaskControl: std::marker::Send + std::marker::Sync + 'static {
        /// Server streaming response type for the SyncNow method.
        type SyncNowStream: tonic::codegen::tokio_stream::Stream<
                Item = std::result::Result<::proto_flow::runtime::SyncNowResponse, tonic::Status>,
            > + std::marker::Send
            + 'static;
        /// SyncNow asks a materialization to immediately commit its open
        /// transaction, and resolves once that transaction is fully acknowledged:
        ...
        async fn sync_now(
            &self,
            request: tonic::Request<::proto_flow::runtime::SyncNowRequest>,
        ) -> std::result::Result<tonic::Response<Self::SyncNowStream>, tonic::Status>;
    }
    /// TaskControl is a user-facing control surface for running tasks.
    /// It's distinct from the Leader service because it has a different
    /// authorization model: callers present ordinary gazette READ claims
    ...
    /// internal hop to the peer front door which is the task's shard-zero primary.
    #[derive(Debug)]
    pub struct TaskControlServer<T> {
        inner: Arc<T>,
        accept_compression_encodings: EnabledCompressionEncodings,
        send_compression_encodings: EnabledCompressionEncodings,
        max_decoding_message_size: Option<usize>,
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for TaskControlServer<T>
    where
        T: TaskControl,
        B: Body + std::marker::Send + 'static,
        B::Error: Into<StdError> + std::marker::Send + 'static,
    {
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/runtime.TaskControl/SyncNow" => {
                    #[allow(non_camel_case_types)]
                    struct SyncNowSvc<T: TaskControl>(pub Arc<T>);
                    impl<T: TaskControl>
                        tonic::server::ServerStreamingService<::proto_flow::runtime::SyncNowRequest>
                        for SyncNowSvc<T>
                    {
                        type Response = ::proto_flow::runtime::SyncNowResponse;
                        type ResponseStream = T::SyncNowStream;
                        type Future =
                            BoxFuture<tonic::Response<Self::ResponseStream>, tonic::Status>;
                        fn call(
                            &mut self,
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut =
                                async move { <T as TaskControl>::sync_now(&inner, request).await };
                            Box::pin(fut)
                        }
                    }
                    let accept_compression_encodings = self.accept_compression_encodings;
    ...
    impl<T> Clone for TaskControlServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    /// Generated gRPC service name
    pub const SERVICE_NAME: &str = "runtime.TaskControl";
    impl<T> tonic::server::NamedService for TaskControlServer<T> {
        const NAME: &'static str = SERVICE_NAME;
    }
}
