use crate::request::new_store_req;
use crate::test_doc::{rand_test_docs, TestDoc};
use crate::tests::{
    assert_checkpoint_eq, build_field_selection, test_collection, MaterializationFixture,
};
use crate::{DriverClientImpl, Fixture};
use protocol::{
    collection::CollectionExt,
    flow::{CollectionSpec, FieldSelection},
    materialize::{ApplyRequest, FenceRequest, SessionRequest, ValidateRequest},
};
...
fn incomplete_store(
    ...
    let checkpoint_desc = String::from_utf8_lossy(&handle).into_owned();

    let s = new_store_req(handle, fields, collection, checkpoint, docs);
    let (tx, rx) = tokio::sync::mpsc::channel::<StoreRequest>(8);

    let handle = tokio::spawn(async move { client.store(s.chain(rx)).await });
...
struct StoreTxnHandle {
    checkpoint_desc: String,
    handle: tokio::task::JoinHandle<Result<tonic::Response<StoreResponse>, tonic::Status>>,
    sender: tokio::sync::mpsc::Sender<StoreRequest>,
}

impl StoreTxnHandle {
    pub async fn complete(self) -> Result<tonic::Response<StoreResponse>, tonic::Status> {
        let StoreTxnHandle {
            checkpoint_desc,
            handle,
            sender,
        ...
        debug!(
            "starting to complete txn for checkpoint: {}",
            checkpoint_desc
        );

        // drop the sender so that the receiver stream will complete.
        std::mem::drop(sender);

        handle.await.expect("failed to join future")
    }
}
*/
