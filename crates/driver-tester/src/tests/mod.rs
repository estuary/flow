...
pub async fn run(test: TestCase, fixture: &mut Fixture) -> TestResult {
    ...
    let result = match test {
        ...
        TestCase::Transactional => transactional::test(fixture).await,
    };
    TestResult {
        name: test.to_string(),
...
impl MaterializationFixture {
    ...
    pub async fn verify_load_docs(
        ...
        let (mut tx, rx) = loader;
        if prev_always_empty != Some(true) {
            tx.send_load(&self.spec, load_docs).await?;
        }
    ...
    pub async fn verify_load(
        ...
        self.verify_load_docs((tx, rx), prev_always_empty, expected, expected)
            .await
    }
}
