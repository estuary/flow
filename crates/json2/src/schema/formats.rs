...
lazy_static::lazy_static! {
    ...
    static ref ISO_8601_DURATION_RE: regex::Regex = regex::Regex::new(r"^P(?:\d+W|(?:(?:\d+Y(?:\d+M)?(?:\d+D)?|\d+M(?:\d+D)?|\d+D)(?:T(?:\d+H(?:\d+M)?(?:\d+S)?|\d+M(?:\d+S)?|\d+S))?)|T(?:\d+H(?:\d+M)?(?:\d+S)?|\d+M(?:\d+S)?|\d+S))$").expect("Is a valid regex");
    ...
    pub fn validate(&self, val: &str) -> bool {
        match self {
            ...
            Self::UriReference | Self::IriReference => {
                iri_string::validate::iri_reference::<iri_string::spec::UriSpec>(val).is_ok()
            }
