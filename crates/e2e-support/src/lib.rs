...
impl DataPlane {
    ...
    pub async fn reset(&self) -> anyhow::Result<()> {
        reset_journals(&self.journal_client, &self.gazette.fragment_root).await
    }
...
/// Delete all journal specs and clear the fragment store.
///
/// This is the implementation behind [`DataPlane::reset`], exposed as a free
/// function so callers that hold pre-cloned handles (e.g. behind a Mutex) can
/// reset without borrowing the DataPlane across await points.
pub async fn reset_journals(
    journal_client: &::gazette::journal::Client,
    fragment_root: &std::path::Path,
) -> anyhow::Result<()> {
    let listed = journal_client
    ...
    std::fs::remove_dir_all(fragment_root).context("failed to remove fragment root")?;
    std::fs::create_dir_all(fragment_root).context("failed to recreate fragment root")?;
