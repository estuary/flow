...
func TestStableWeightsRegression(t *testing.T) {
	...
	assert.Equal(t, expect, generateStableWeights(len(expect)))
	// They're also invariant to length (weights N is a prefix of M for N < M).
	assert.Equal(t, expect[:4], generateStableWeights(4))
	// They're also invariant to length (weights N is a prefix of M for N < M).
	assert.Equal(t, expect[:12], generateStableWeights(12))
	...
	assert.Equal(t, 490, total) // Ideal is 512 (32 * 16).
...
func TestRankingCases(t *testing.T) {
	...
	for _, tc := range cases {
		...
		assert.Equal(t, tc.expect, r.pick(tc.hash, tc.clock))
	}

	// Switch from "broadcast" to "choose" mode.
	cfg.BroadcastTo, cfg.ChooseFrom = cfg.ChooseFrom, cfg.BroadcastTo
	r = newRendezvous(cfg)
