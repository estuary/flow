...
-- after at most 25 hours. But controllers cannot do so for config updates that are
...
-- clean up any config updates don't get deleted by the normal process. Config updates are
...
create trigger on_config_update_insert after insert on public.config_updates
for each row
