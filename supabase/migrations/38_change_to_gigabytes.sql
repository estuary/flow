...
gbs_by_month as (
  select
    tenants.tenant as tenant,
    ts,
    ceil(sum((catalog_stats_monthly.bytes_written_by_me + catalog_stats_monthly.bytes_read_by_me) / (10.0^9.0))) as monthly_usage_gbs
  from catalog_stats_monthly
  join tenants on catalog_stats_monthly.catalog_name ^@ tenants.tenant
  where tenants.trial_start is null
  group by tenants.tenant, ts
  having ceil(sum((catalog_stats_monthly.bytes_written_by_me + catalog_stats_monthly.bytes_read_by_me) / (10.0^9.0))) > 10
)
select
    tenants.tenant as tenant,
