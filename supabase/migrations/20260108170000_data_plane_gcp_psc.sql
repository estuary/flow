-- Add GCP Private Service Connect endpoints column to data_planes
-- and update the overview view to include it

BEGIN;

ALTER TABLE public.data_planes ADD gcp_psc_endpoints JSON[];
GRANT SELECT(gcp_psc_endpoints) ON public.data_planes TO authenticated;

-- Recreate data_planes_overview to include gcp_psc_endpoints.

-- First drop the view since CREATE OR REPLACE VIEW can't change the column
-- order of an existing view.
DROP VIEW IF EXISTS public.data_planes_overview;

-- The rest is a copy of 20251217004145_data_planes_overview_view.sql with one addition:
--   dp.gcp_psc_endpoints (added after dp.azure_link_endpoints)
...
SELECT
    ...
    dp.gcp_psc_endpoints,  -- Added: GCP Private Service Connect endpoints
    dp.enable_l2
