...
-- are not blocked here; their grants will be handled separately
...
exception when others then
  -- Fail closed: deny the token rather than letting a broken hook silently
  -- pass through. The error message surfaces in GoTrue's auth logs.
  return jsonb_build_object(
    'error', jsonb_build_object(
      'http_code', 500,
      'message', 'check_sso_requirement failed: ' || sqlerrm
    )
  );
end;
$$;
