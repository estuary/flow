...
-- resolves.
create function internal.has_controller_alert(status json) returns boolean
language sql immutable
as $$
    select exists (
        select 1
        from json_each(json_extract_path(status, 'alerts'))
    );
$$;
comment on function internal.has_controller_alert(json) is
'Returns true if the controller status JSON contains any alerts,
 used for the has_alert computed column';

alter table public.controller_jobs
add column has_alert boolean generated always as (internal.has_controller_alert(status)) stored;
  ...
  begin
    ...
    resolving_alerts as (
     ...
     update public.alert_history
       ...
       where alert_history.alert_type = ra.alert_type
         and alert_history.catalog_name = ra.catalog_name
         and alert_history.fired_at = ra.fired_at;
