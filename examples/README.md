...
Note that these examples are tested with `flowctl test`, which runs a catalog's
tests entirely locally. That is distinct from `flowctl catalog test`, which runs
tests on the server side and performs authorization checks (those would fail
here, because users don't have access to these prefixes in the Estuary
