...
class DiscoverRequest(BaseModel):
    """Discovery is static here, so the configuration it carries is unused."""
...
class Request(BaseModel):
    ...
    Extra properties are retained rather than ignored, so a variant this
    connector doesn't handle can be named in the error it raises.
    """

    # Validation failures can concern the decrypted endpoint configuration.
    # Pydantic otherwise includes the rejected input value in its exception,
    # which the connector process would emit to task logs on an uncaught error.
    model_config = ConfigDict(
        populate_by_name=True, extra="allow", hide_input_in_errors=True
    )
