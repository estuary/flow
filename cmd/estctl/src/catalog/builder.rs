...
impl Builder {
    ...
    fn query_collection(&self, name: &str, _from_resource_id: i64) -> Result<i64, Error> {
        let id = self.db.prepare_cached("
            ...
            .map_err(|e| QueryCollectionErr {name: name.to_owned(), detail: e} )?;
    ...
    fn process_transform(&self, rid: i64, target_id: i64, transform: &specs::Transform) -> Result<i64, Error> {
        ...
        let (typ, body, body_rid, bootstrap, bootstrap_rid) = match &transform.lambda {
            ...
            JqBlock(body) => {
                ("jq", Some(body.clone()), Some(rid), None, None)
            },
            Sqlite {
                bootstrap,
                body,
                ...
                if let Some(bootstrap) = bootstrap {
                    ...
                    ("sqlite", Some(body), Some(body_rid), Some(bootstrap), Some(bootstrap_rid))
                } else {
                    let (body, body_rid) = deref(body)?;
            ...
            SqliteBlock {
                ...
                if let Some(bootstrap) = bootstrap {
                    ("sqlite", Some(body.clone()), Some(rid), Some(bootstrap.clone()), Some(rid))
                } else {
        ...
        let lambda_id = self.db.prepare_cached("SELECT last_insert_rowid();")?
        ...
        let (shuf_key, shuf_broadcast, shuf_choose) = match &transform.shuffle {
            Some(shuffle) => {
                (
                    shuffle.key.as_ref().map(|k| serde_json::to_string(&k).unwrap()),
                    shuffle.broadcast,
                    shuffle.choose,
                )
            },
