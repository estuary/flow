...
func (ps *ProxyServer) Proxy(streaming pf.NetworkProxy_ProxyServer) error {
	...
	ops.PublishLog(container.logger, ops.Log_info, "opening inbound connection to container",
	...
	defer func() {
		...
		ops.PublishLog(container.logger, ops.Log_info, "proxy connection closed",
			"clientAddr", open.ClientAddr,
			"targetPort", open.TargetPort,
	...
	ops.PublishLog(container.logger, ops.Log_debug, "proxy connection opened", "port", open.TargetPort, "clientIP", open.ClientAddr)
	...
	if err != nil {
		status = "error"
		ops.PublishLog(container.logger, ops.Log_warn, "proxy connection failed", "port", open.TargetPort, "clientAddr", open.ClientAddr, "error", err)
	} else {
		ops.PublishLog(container.logger, ops.Log_debug, "proxy connection closing normally", "port", open.TargetPort, "clientAddr", open.ClientAddr)
	}
