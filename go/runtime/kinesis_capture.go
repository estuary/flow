...
// Represents a specific task term for the capture, along with associated data that
// needs re-initialized on every new term. Each new term initiates an entirely new
type kinesisTerm struct {
	// The task revision, used to differentiate messages coming from old revisions so they can be
	// ignored.
	revision    int64
	mapper      flow.Mapper
...
// Send a command to ourselves to initiate a new term. Terms are initialized within ConsumeMessage
// to avoid synchronization issues. This just sends the message, but does not wait for the new term
// to be initialized.
...
// Called from ConsumeMessage to initialize a new term. Ends any current term immediately.
