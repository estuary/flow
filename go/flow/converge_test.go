...
func TestConvergence(t *testing.T) {
	...
	t.Run("partitions-split-errors", func(t *testing.T) {
		...
		// Case: not a power-of-two >= 2.
		_, err = MapPartitionToSplit(collection, allPartitions[0:1], 3)
		require.EqualError(t, err, "splits must be a power of two and >= 2")
		_, err = MapPartitionToSplit(collection, allPartitions[0:1], 1)
		require.EqualError(t, err, "splits must be a power of two and >= 2")
	})
