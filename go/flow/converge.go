...
// CollectionChanges compares a CollectionSpec and |desired| splits with the
// collection's current |partitions|, and appends proposed JournalChanges
// which bring the current state into consistency with the desired state.
func CollectionChanges(
	collection *pf.CollectionSpec,
	partitions []pb.ListResponse_Journal,
	desired []pf.LabelSet,
	into []pb.ApplyRequest_Change,
) ([]pb.ApplyRequest_Change, error) {
	var idx = make(map[pf.Journal]*pb.ListResponse_Journal, len(partitions))
	for i := range partitions {
		idx[partitions[i].Spec.Name] = &partitions[i]
	}
...
// TaskChanges compares a Task and |desired| splits with the tasks's current
// |shards| and |recoveryLogs|, and appends proposed shard and journal changes
// which bring the current state into consistency with the desired state.
func TaskChanges(
	task pf.Task,
	shards []pc.ListResponse_Shard,
	recoveryLogs []pb.ListResponse_Journal,
	desired []pf.LabelSet,
	intoShards []pc.ApplyRequest_Change,
	intoJournals []pb.ApplyRequest_Change,
) ([]pc.ApplyRequest_Change, []pb.ApplyRequest_Change, error) {

	var shardIdx = make(map[pc.ShardID]*pc.ListResponse_Shard, len(shards))
	var logIdx = make(map[pb.Journal]*pb.ListResponse_Journal, len(recoveryLogs))

	for i := range shards {
		shardIdx[shards[i].Spec.Id] = &shards[i]
	}
	for i := range recoveryLogs {
		logIdx[recoveryLogs[i].Spec.Name] = &recoveryLogs[i]
	}
