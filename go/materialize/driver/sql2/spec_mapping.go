...
func TableForMaterialization(name string, comment string, identifierQuotes *TokenPair, spec *pf.MaterializationSpec) *Table {
	return &Table{
		Identifier: toIdentifier(name, identifierQuotes),
		Comment:    comment,
		Columns:    columnsForMaterialization(spec, identifierQuotes),
	}
}

var unquotedIdentifiers = regexp.MustCompile("^[_a-zA-Z]+[_\\pL\\pN]*$")

func toIdentifier(name string, quotes *TokenPair) string {
	if unquotedIdentifiers.MatchString(name) {
		return name
	}
	return quotes.Wrap(name)
}
...
// statements.
func columnsForMaterialization(spec *pf.MaterializationSpec, identifierQuotes *TokenPair) []Column {
	...
	for _, field := range allFields {
		...
		columns = append(columns, ColumnForProjection(projection, identifierQuotes))
	}
	return columns
}

func ColumnForProjection(projection *pf.Projection, identifierQuotes *TokenPair) Column {
	var column = Column{
		Name:       projection.Field,
		Identifier: toIdentifier(projection.Field, identifierQuotes),
...
func generateApplyStatements(endpoint *Endpoint, spec *pf.MaterializationSpec) ([]string, error) {
	...
	var comment = endpoint.Generator.Comment(fmt.Sprintf(
		...
		endpoint.Tables.TargetName,
	))
	var target = TableForMaterialization(endpoint.Tables.TargetName, comment, &endpoint.Generator.IdentifierQuotes, spec)

	createCheckpoints, err := endpoint.Generator.CreateTable(endpoint.Tables.Checkpoints)
	if err != nil {
		return nil, fmt.Errorf("generating checkpoints schema: %w", err)
	}
	createSpecs, err := endpoint.Generator.CreateTable(endpoint.Tables.Specs)
	if err != nil {
	...
	insertSpec, err := endpoint.Generator.DirectInsertStatement(
		endpoint.Tables.Specs, endpoint.Tables.TargetName, base64.StdEncoding.EncodeToString(specBytes))
	if err != nil {
