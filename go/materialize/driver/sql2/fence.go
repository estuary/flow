	...
	if _, err = txn.Exec(
		fmt.Sprintf(`
			UPDATE %s
				...
				AND (
					(key_begin<=%s AND key_end>=%s) OR
					(key_begin<=%s AND key_end>=%s) OR
					(key_begin>%s AND key_begin<%s)
				)
			;
			`,
			...
			e.Generator.Placeholder(5),
			e.Generator.Placeholder(6),
		),
		materialization,
		keyBegin, keyBegin, // Does the table row cover |keyBegin|?
		keyEnd, keyEnd, // Does the table row cover |keyEnd|?
		keyBegin, keyEnd, // Does [keyBegin, keyEnd] cover the table row key_begin ?
	); err != nil {
