...
// TaskServiceV2 wraps the runtime-next CGO `TaskService`. Its lifecycle and
// pipe-FD log plumbing mirrors `TaskService` exactly; the symbols differ so
// the two runtimes can be linked side-by-side during the V2 rollout.
type TaskServiceV2 struct {
	config pr.TaskServiceConfig
	cSvc   *C.TaskServiceV2
	conn   *grpc.ClientConn
	lwaCh  <-chan struct{}
}

func NewTaskServiceV2(
	config pr.TaskServiceConfig,
	...
	logrus.WithFields(logrus.Fields{
		...
		"log_fd_write": wDescriptor,
		"log_fd_read":  logReader.Fd(),
	...
	go func() {
		...
		if err != nil {
			...
			}).Error("failed to process cgo V2 service channel logs")
		}
	}()

	udsFile, err := os.CreateTemp("", "run-sock")
	if err != nil {
		return nil, fmt.Errorf("creating task service socket file: %w", err)
	}
	config.UdsPath = udsFile.Name()

	// Rust service will re-create the path.
	if err = udsFile.Close(); err != nil {
		return nil, fmt.Errorf("closing task service socket file: %w", err)
	}
	...
	var svc = &TaskServiceV2{
		config: config,
		cSvc: C.new_task_service_v2(
...
func (s *TaskServiceV2) Conn() *grpc.ClientConn {
	return s.conn
}

func (s *TaskServiceV2) Drop() {
	if s.conn != nil {
		_ = s.conn.Close()
		s.conn = nil
	}
	if s.cSvc != nil {
		C.task_service_v2_drop(s.cSvc)
		s.cSvc = nil
	}
	if s.lwaCh != nil {
...
func (s *TaskServiceV2) err() error {
	var err error
	if s.cSvc.err_len != 0 {
