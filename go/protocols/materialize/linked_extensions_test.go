...
func TestRequestValidateLinkedCollections(t *testing.T) {
	...
	// Two bindings of one collection with differing `group_by` intern as two
	// same-name entries which differ in `key` and `is_primary_key`. Duplicate
	// names are legal: only the index identifies an entry.
	var keyedOnOther = linkedTestCollection("acmeCo/one")
	keyedOnOther.Key = []string{"/other"}
	keyedOnOther.Projections = []pf.Projection{
		{Field: "id", Ptr: "/id"},
	...
	indirect.Bindings = []*Request_Validate_Binding{
		{
			ResourceConfigJson: []byte(`{"table":"by_id"}`),
			GroupBy:            []string{"id"},
		},
		{
			ResourceConfigJson: []byte(`{"table":"by_other"}`),
			CollectionIndex:    1,
			GroupBy:            []string{"other"},
		},
	}
	require.NoError(t, indirect.Validate())
	require.Equal(t, []string{"/id"}, indirect.BindingCollection(indirect.Bindings[0]).Key)
	require.Equal(t, []string{"/other"}, indirect.BindingCollection(indirect.Bindings[1]).Key)
