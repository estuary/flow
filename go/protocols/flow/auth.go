...
const (
	...
	// PROXY gives the bearer a capability to proxy a TCP to a connector.
	Capability_PROXY pb.Capability = 1 << 18
)
...
func (a *authServer) Proxy(stream NetworkProxy_ProxyServer) error {
	if ctx, cancel, claims, err := a.verifier.Verify(stream.Context(), Capability_PROXY); err != nil {
		return err
	} else {
		defer cancel()
