...
func (g *governor) next(ctx context.Context) (message.Envelope, error) {
	for {
		...
		if r.readDelay != 0 && readTime > g.wallTime {
			...
			r.log(ops.DebugLevel, "gated documents of journal", "until", readTime)
			continue
		}
	...
	for r := range g.pending {
		...
		if err := r.onRead(result, ok); err != nil {
			...
			if err != context.Canceled {
				r.log(ops.WarnLevel, "shuffled read failed (will retry)", "error", err)
			} else {
				r.log(ops.DebugLevel, "shuffled read has drained")
			}
...
func (g *governor) onTick() error {
	...
	for _, r := range g.gated {
		heap.Push(&g.queued, r)
		g.setPollState(r, pollStateReady)
		r.log(ops.DebugLevel, "un-gated documents of journal", "now", g.wallTime)
	}
	g.gated = g.gated[:0]
...
func (g *governor) onConverge(ctx context.Context) error {
	...
	for _, r := range drain {
		r.log(ops.DebugLevel, "cancelled shuffled read marked for draining")
		r.cancel()
	}
