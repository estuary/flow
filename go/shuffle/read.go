...
func startWatches(ctx context.Context, jc pb.JournalClient, shuffles []shuffle) (chan error, error) {
	...
	for i := range shuffles {
		shuffles[i].listing = client.NewWatchedList(ctx, jc,
			pb.ListRequest{
				Selector: pb.LabelSelector{
					Include: pb.MustLabelSet(
						"name:prefix", shuffles[i].sourceSpec.PartitionTemplate.Name.String()+"/",
						labels.Collection, shuffles[i].sourceSpec.Name.String(),
					),
				},
				Watch: true,
			},
...
// are known but explicitly filtered by the shuffle.
