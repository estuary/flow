...
func NewDriver(connectorNetwork string) pc.DriverServer {
	return driver{connectorNetwork: connectorNetwork}
}
...
func (d driver) Spec(ctx context.Context, req *pc.SpecRequest) (*pc.SpecResponse, error) {
	...
	var err = RunConnector(ctx, source.Image, d.connectorNetwork,
		[]string{"spec"},
...
func (d driver) Capture(req *pc.CaptureRequest, stream pc.Driver_CaptureServer) error {
	...
	if err := RunConnector(stream.Context(), source.Image, d.connectorNetwork,
		invokeArgs,
		invokeFiles,
		// No stdin is sent to the connector.
		func(w io.Writer) error { return nil },
