...
// container into base directory "/opt/connector".
//
...
func RunConnector(
	...
	for name, m := range jsonFiles {
		...
		} else if err = os.WriteFile(filepath.Join(tempdir, name), content, 0600); err != nil {
	...
	args = append([]string{
		...
		"--rm",
		"--mount",
		fmt.Sprintf("type=bind,source=%s,target=/opt/connector", tempdir),
		image,
	}, args...)
