...
func Run(
	...
	if connectorProtocol == FlowMaterialize {
		...
		if err := pullImage(ctx, image, logger); err != nil {
			...
			imageArgs = append(imageArgs, "--mount", fmt.Sprintf("type=bind,source=%[1]s,target=%[1]s", inspectOutput))
			jsonFiles["image_inspect_results.json"] = inspectOutput
		}

		args = append(args,
			"--image-inspect-json-path=/tmp/image_inspect_results.json",
