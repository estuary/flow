...
export type TransformLambda = (doc: any, ctx: StateStore) => any[] | Promise<any[]> | void | Promise<void>;
export type BootstrapLambda = (state: StateStore) => void | Promise<void>;

export interface TransformMap { [id: number]: TransformLambda }
export interface BootstrapMap { [id: number]: BootstrapLambda }
...
export class StateStore {
    ...
    set(key: string, doc: any, expiry?: Date): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            ...
            req.on('data', () => { return; } ); // We expect no response.
    ...
    _get(key: string, prefix: boolean): Promise<any> {
        const query = querystring.encode({key: key, prefix: prefix});
        const req = this.session.request({
            ':method': 'GET',
            ':path': '/docs?' + query,
...
class Server {
    ...
    _processTransformStream(req: h2.ServerHttp2Stream, hdrs: h2.IncomingHttpHeaders, _: number): void {
