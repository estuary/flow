...
impl Service {
    ...
    pub async fn transform(
        ...
        transform_id: i64,
        store: &url::Url,
    ) -> Result<(hyper::body::Sender, impl TryStream<Ok = bytes::Bytes, Error = Error>), Error>
    {
        ...
        let foobar = futures::stream::try_unfold(
            resp.into_body(),
            |mut body| async {
    ...
    fn start(db: &DB, dir: tempfile::TempDir) -> Result<Service, Error> {
        ...
        // TODO(johnny) why is this logging to stdout??? should be captured.
