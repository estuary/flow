...
fn verify_are_documents_ordered(collection_key: Vec<String>, documents: &[Value]) -> Result<()> {
    ...
    if !is_sorted {
        ...
        Err(TestExpectationsOutOfOrder {
            collection_key,
...
#[derive(Debug)]
pub struct TestExpectationsOutOfOrder {
    collection_key: Vec<String>,
    reordered_documents: Vec<Value>,
}

impl Display for TestExpectationsOutOfOrder {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln!(f, "The expected documents must be provided in lixicographic order according to the collection key: [{}]",
...
impl std::error::Error for TestExpectationsOutOfOrder {}
...
mod test {
    ...
    use crate::catalog::specs::{TestStep, TestStepVerify};
