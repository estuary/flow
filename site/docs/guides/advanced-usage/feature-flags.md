...
- **Behavior:** Adds `x-infer-schema: true` to the collection schema, telling Estuary to infer schema from captured documents alongside the connector's discovered column types.
...
Emits schema updates to Estuary during capture based on the source database's current schema.

- **Default:** Enabled for new captures. Existing captures may have `no_emit_sourced_schemas` set.
- **Behavior:** The connector periodically sends SourcedSchema messages containing the authoritative schema from the source database. Estuary uses these to update collection schemas without requiring manual re-discovery.
...
- **Applies to:** PostgreSQL, MySQL, Snowflake, BigQuery, and other SQL materialization connectors.
...
| `no_snowpipe_streaming` | Use regular Snowpipe instead of Snowpipe Streaming. Snowpipe Streaming is the default when delta updates is enabled. | Snowflake |
