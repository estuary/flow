...
This page introduces concepts and terms that will help you begin working with Estuary and better understand its underlying mechanisms.
These concepts are discussed in more detail on subsequent pages.

![Estuary at a glance](./concept-images/at-a-glance.png)

## Working with Estuary

There are two main environments in which you can work with Estuary:
the web application or your preferred local environment using the flowctl command-line tool.

### Web application

Estuary's web application is where you'll most likely create, monitor, and update your Data Flows.
...
flowctl is a command-line interface for working with Estuary's public API.
...
*In this section, you'll find the most important terms and concepts for Estuary. If you're new to Estuary, start here.*

A complete end-to-end **Data Flow** between two systems has three components:

* **Capture**: Estuary ingests data from an outside source.
* **Collections**: Estuary maintains the captured data in cloud storage.
* **Materialization**: Estuary pushes data to an outside destination.
...
All catalog entities are defined in **Data Flow specification files** — either manually, by you, or generated by Estuary's web app. You create the specifications as **drafts**, and **publish** them to add them to the catalog.
...
**Collections** represent datasets within Estuary.
...
A **capture** is an Estuary task that ingests data from an external source into one or more collections.
Documents continuously move from the source into Estuary;
as new documents become available at the source,
Estuary validates their schema and adds them to their corresponding collection.
...
**Endpoints** are the source systems from which Estuary captures data and the destination systems to which Estuary materializes data. All kinds of data systems can be endpoints, including databases, key/value stores, streaming pub/sub systems, SaaS products, and cloud storage locations.

Estuary connects to this wide variety of endpoints using **connectors**.

***

### Connectors

**Connectors** are plugin components that allow Estuary to interface with endpoint data systems.
They power captures and materializations.

Estuary uses an open-source connector model.
Many connectors are made by Estuary, and others are made by third parties.
Because connectors are open-source and kept separate from Estuary itself,
...
All currently supported connectors are ready to use in Estuary's web application.
...
All collections in Estuary have an associated
[JSON schema](https://json-schema.org/understanding-json-schema/)
against which documents are validated every time they're written or read.
Schemas are critical to how Estuary ensures the integrity of your data.
Estuary validates your documents to ensure that
...
Estuary pauses catalog tasks when documents don't match the collection schema,
...
Every collection [schema](#schemas) in Estuary includes a  **key**.
...
Estuary also uses the key to **reduce** documents in collections, making storage and materializations more efficient.
...
Estuary's [collections](#collections) use cloud storage buckets for the durable storage of data.
Storage mappings define how Estuary maps your various collections into your storage buckets and prefixes.
...
*This section discusses advanced Estuary concepts. The information here unlocks
a more technical understanding of how Estuary works, and may be helpful in advanced use cases.*

### Journals

**Journals** provide the low-level storage for Estuary's collections.
...
Shards are created and managed by the Estuary runtime.
...
Estuary leverages your JSON schemas to produce other types of schemas as needed,
