...
Estuary processes and stores data in terms of documents: JSON files that consist of multiple key-value pair objects. Collections are comprised of documents; Estuary's tasks (captures, materializations, and derivations) process data in terms of documents.

A document corresponds to different units of data in different types of endpoint systems.
...
Estuary allows documents up to 16 MB in size, but it's rare for documents to approach this limit.
...
The `_meta` object is present in all documents, and contains metadata added by Estuary.
...
The `_meta/uuid` field is a system-generated globally unique identifier for each document within Estuary.

#### flow_published_at

The `flow_published_at` field is a system-generated timestamp within Estuary,
...
- Source: The `flow_published_at` field is generated by the runtime environment of Estuary.
...
- A document in Estuary is any JSON object emitted by a capture connector.
...
Sign into Estuary's web application and click the **Collections** tab. The collections to which you have access are listed.
...
In your [authenticated flowctl session](/reference/authentication/#authenticating-estuary-using-the-cli), issue the command `flowctl collections read --collection <full/collection-name>`. For example, `flowctl collections read --collection acmeCo/inventory/anvils`.

Options are available to read a subset of data from collections.
For example, `--since` (a relative duration like `10m`) or `--not-before` (an absolute RFC-3339 timestamp)
let you specify an exact start time from which to read data, and
`--partitions` allows you to read only data from a specified [logical partition](../concepts/advanced/projections.md#logical-partitions).
By default the command reads all currently-available data and then exits; pass `--follow` to keep reading new data as it's written.
Use `flowctl collections read --help` to see documentation for all options.

## Specification

Collections are defined in Data Flow specification files per the following format:
...
Every Estuary collection must declare a schema,
...
For many source types, Estuary is able to generate a basic schema during [discovery](./captures.md#discovery).
...
Every Estuary collection must declare a `key` which is used to group its documents.
...
Estuary performs static inference of the collection schema to verify the existence
...
Estuary itself doesn't mind if a keyed location has multiple types,
...
A collection key instructs Estuary how documents of a collection are to be
reduced, such as while being materialized to an endpoint.
Estuary also performs opportunistic local reductions over windows of documents
...
arrive so closely together that they were locally combined by Estuary.
...
determine how Estuary's collections are mapped into
