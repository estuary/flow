...
Each organization that uses Estuary has a `logs` collection under the global `ops` prefix.
...
Logs are collected from events that occur within Estuary's runtime,
...
* `error`: Non-recoverable errors from Estuary's runtime or connector that are critical to know about
...
Each organization that uses Estuary has a `stats` collection under the global `ops` prefix.
...
* Compare a data throughput of a task between platforms; for example, compare reported data capture by Estuary to detected change rate in a source system.
