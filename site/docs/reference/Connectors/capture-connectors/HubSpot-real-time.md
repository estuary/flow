...
Note that calculated property refreshes emit partial documents containing only the record's key and calculated property values. By default, these are combined with previously captured complete documents, and this works well for [standard updates materializations](/concepts/materialization/#how-continuous-materialization-works). However, [delta updates materializations](/concepts/materialization/#delta-updates) do not fully reduce documents, so partial documents from calculated property refreshes are materialized as-is with all non-calculated properties as `null`.
