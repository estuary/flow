...
1. In your SendGrid dashboard, navigate to **Settings** > **Mail Settings**. Under **Webhook Settings**, select **Event Webhooks**.

2. Click **Create new webhook** (or click the cog icon to edit an existing one). In the **Post URL** field, paste the webhook URL generated by your Estuary capture. See the [Webhook URLs](#webhook-urls) section below for how to determine your full URL.

3. Select the event types you'd like to track (e.g. Delivered, Opened, Clicked, Bounced, etc.).

4. Under **Security features**, enable the **Enable Signed Event Webhook** toggle. Click **Save** to generate the key pair.

5. Re-open the webhook configuration by clicking the cog icon. The public verification key will now be displayed under the signature toggle. Copy this key.

6. In your Estuary capture configuration, paste the verification key into the `signatureConfig.publicKey` field with provider set to `"twilio"`. See [Webhook signature verification](#webhook-signature-verification) below.
