...
This connector materializes Flow collections into tables in a Starburst Galaxy.

The connector first uploads data changes to a S3 AWS storage.
From there, it transactionally applies the changes to the Starburst tables.
...
* A Starburst Galaxy account (To crate one: [Staburst Galaxy start](https://www.starburst.io/platform/starburst-galaxy/start/) that includes:
  * Cluster with catalog on S3
  * A schema, to which you'll materialize data
  * Storage on S3 for temporary data with `awsAccessKeyId` and `awsSecretAccessKey`
