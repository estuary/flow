...
Estuary collections into a GCS bucket in the Apache Parquet format.

The delta updates are batched within Estuary, converted to Parquet files, and then pushed to the GCS bucket
...
materializations:
  ${PREFIX}/${mat_name}:
    endpoint:
      connector:
        image: "ghcr.io/estuary/materialize-gcs-parquet:v1"
        config:
          bucket: bucket
